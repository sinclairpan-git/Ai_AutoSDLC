...
生成的 `src/pages/`、历史 `src/views/` 和 `src/components/business/` 不得直接 `import ... from 'primevue/*'`。该边界必须可通过 import scan 或 AST scan 验收。
...
新项目默认生成 `src/pages/`，不得同时生成 `src/pages/` 和 `src/views/`；历史项目若已经使用 `src/views/`，则沿用旧根目录，不额外引入 `src/pages/`。
...
Given 生成业务页面，When 扫描 `src/pages/`、历史 `src/views/` 和 `src/components/business/`，Then 不应存在 `from 'primevue/*'` 的直接 import；页面应优先依赖语义组件、Base 组件或受控业务组件。
...
- 若带单测模板或既有项目已经保留 Vitest，则维持其单测基线；普通用户默认展示文案不把 Vitest 作为必选项。
