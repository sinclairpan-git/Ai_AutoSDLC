...
**文档状态**：已归档，三轮对抗评审通过  
**创建日期**：2026-06-22  
**最近更新**：2026-07-03，吸收 `Vue3企业级前端开发规范方案 v1.6.1（AI 执行版）`
**适用仓库**：Ai_AutoSDLC  
...
6. 将 v1.6.1 规范中的视觉家族、Base/Business/View 分层、开发期 `$i('中文')` 国际化、前端子项目工程化边界、路由 meta、公共 API 响应类型、TypeScript unknown 优先和提交粒度规则沉淀为可执行治理约束。
...
当前 Vue3 managed scaffold / 带单测模板的开发依赖继续覆盖：
...
v1.6.1 面向用户的默认验证口径以 Playwright、ESLint、Prettier、husky、lint-staged、commitlint 为主；若模板保留 Vitest 或既有项目存在单测体系，行为变更必须补齐对应单测，但不得把“必须引入 Vitest”作为所有普通生成项目的硬约束。
...
7. `definePreset` 必须同时覆盖 `primary`、`surface`、`highlight` 三类语义，避免只配置主色而把壳层、导航和高亮交给页面 CSS 兜底
...
接口命名以后台实际契约、生成代码或既有服务层约定为主；在无既有约定时可优先使用业务动词，例如 `getUserList`、`createUser`、`updateUser`、`deleteUser`。若后端存在统一响应包装，默认模板必须在 `api/types.ts` 或 `types/` 中提供公共泛型结构，例如 `ApiResponse<T>`，避免页面局部重复声明响应壳。
...
9. 缺失 `primary/surface/highlight` 主题语义、路由 `meta.title/auth/keepAlive/roles`、公共 API 响应泛型、Base 层权限职责、深色信息块边界或提交粒度说明时，必须进入可观测治理证据。
...
| 原生 `select` 与 PrimeVue 输入框混用、`contrast` 标签滥用、导航/内容区视觉割裂 | warning | v1.3 起作为视觉一致性证据，v1.6.1 继续保留，不阻断普通生成 |
| 原生 `input` 与 PrimeVue 输入框混用、缺失主题 `surface/highlight` 语义、缺失路由 meta 或公共响应泛型、深色信息块误用于普通卡片/筛选区 | warning | v1.6.1 新增可观测治理项，先进入证据而不阻断普通生成 |
