...
## FD-2026-08-31-001 | 两轮评审后缺少终局 Sponsor 分支，导致关闭流程递归

- 日期 (UTC): 2026-08-31
- 来源: user_report, adversarial_review
- 状态: open
- owner: codex
- wi_id: 225-review-terminal-sponsor-convergence
- related_doc: specs/225-review-terminal-sponsor-convergence/spec.md
- 现象: 同一候选经过两轮修复/复审后，GitHub PR heartbeat 仍会继续申请“最后一次例外”或建议再开 records-only PR；WI224 产品和主线已经完成后，仍差点为 lifecycle close-check 创建 #196。相邻的 Local PR Review runtime 达到上限时虽返回 `needs_user`，`next_action` 仍提示增加 `--max-rounds`。
- 触发场景: Codex review 或 post-merge close-check 在已冻结收口边界后继续提出记录/continuity finding，而 Local Repository PR Protocol 只规定持续监控至合并/用户 blocker，没有定义两轮后的 terminal sponsor decision。
- 影响范围: PR/CI 成本、治理记录膨胀、ROI、候选终态可信度，以及“为证明关闭而继续创建关闭 PR”的递归风险。
- 根因分类: A, D, H
- 未来杜绝方案摘要: 两轮后暂停 heartbeat，Sponsor 只做一次 `stop` 或 `approve-one-bounded-action` 决策；批准必须同时冻结 `unique_delta`、`effort_cap`、`terminal_outcome`。终局复核只验证稳定 finding、冻结 delta 和直接回归；新高风险事实只能把终局结果改为 No-Go/blocked，不能自动增加轮次。
- 建议改动层级: prompt / context, rule / policy, workflow, eval
- prompt / context: 根 `AGENTS.md` 的 Local Repository PR Protocol 必须明确两轮上限、暂停 heartbeat、一次 Sponsor 决策和三项冻结字段。
- rule / policy: 普通修复/复审不得超过两轮；terminal sponsor action 结束后无论 PASS、known-blocked 或 No-Go 都停止，不得申请第二个例外。
- middleware: not-applicable；本次复发层是仓库自身代理协议，WI225 不授权新增或修改 runtime middleware。
- workflow: 达到上限时先暂停 heartbeat；Sponsor 拒绝则记录真实终态并删除自动化，批准则只执行一个冻结 delta 和一次终局复核。
- tool: `AGENTS.md`, Codex heartbeat automation, existing finding history and PR review evidence
- eval: 两轮后新增普通修复轮次次数、同一 WI 的 post-merge records PR 数量、未冻结三字段即恢复 heartbeat 的次数、terminal action 后再次申请例外的次数。
- 风险等级: 高
- 下一步任务归属: WI225 只完成 formal/admission；唯一候选是根 `AGENTS.md` 的 rules-only 改动，总投入不超过 0.5 人日，需在 Formal PR 合并后重新获得 execute 授权。
- 可验证成功标准: 给定同一候选已完成两轮修复/复审，repo-local 协议要求暂停而非继续；批准路径同时具备 `unique_delta / effort_cap / terminal_outcome`，且终局复核后不能启动下一轮。
- 是否需要回归测试补充: 后续 rules-only 候选以 constraints、规则标记检查和独立对抗评审验证；本 Formal 不修改规则实现。
