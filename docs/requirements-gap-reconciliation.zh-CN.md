...
- **正式口径声明**：本轮补齐采用“原 PRD 保持不动 + 缺口总表集中分流 + `001/002` 按原范围回写 + 新 spec 承接边界外内容”的结构。其中特别约定：`003` 只承接**原 PRD 已有、但不适合继续塞进 `001` / `002`** 的跨域旧债；`004` 只承接**原 PRD 之外、但仓库已实现并形成事实标准**的后续新增能力。因此，严格意义上，“新 spec 只接跨模块、原 PRD 有但 `001` / `002` 不适合继续塞进去的内容”这一条，只对应 `003`，不对应 `004`。
