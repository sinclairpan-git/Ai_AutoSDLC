...
**Architecture:** Keep the existing merge-base and execution-log history algorithms. Before classifying branch-local changed paths, require the diff to contain a recognized canonical or scoped control file for the requested work item; otherwise treat the branch-local path set as unattributed and empty. Arbitrary descendants such as `notes.md` are not attribution anchors.
...
### Task 2: Address Codex review attribution bypass

**Files:**
- Modify: `tests/integration/test_cli_workitem_truth_check.py`
- Modify: `src/ai_sdlc/core/workitem_truth.py`

- [x] **Step 1: Verify the review finding**

An unrelated branch can add `specs/<historical-wi>/notes.md` plus unrelated source code. A directory-prefix anchor preserves the entire diff and incorrectly reports execution for the historical WI.

- [x] **Step 2: Add and observe the focused RED regression**

`test_truth_check_ignores_auxiliary_note_under_historical_work_item` failed with `branch_only_implemented` before the production change.

- [x] **Step 3: Restrict the anchor to recognized WI controls**

Use exact canonical `spec.md`, `plan.md`, `tasks.md`, `task-execution-log.md`, or the exact scoped `codex-handoff.md`. Do not accept arbitrary descendants of either directory.

- [x] **Step 4: Re-run focused and repository truth verification**

Results:

- work-item truth tests: `43 passed`
- Program Truth snapshot tests: `13 passed, 403 deselected`
- repository manifest truth test: `1 passed`
- Ruff: pass
- constraints: no blockers
- WI095/WI121 on `origin/main` and `HEAD`: `formal_freeze_only`, empty changed/code paths
- Program Truth dry-run: blocked, inventory `1149/1149`, unmapped `0`, missing `1`
