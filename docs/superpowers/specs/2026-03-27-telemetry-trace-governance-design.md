...
为避免 implementation plan 还要回翻上游 spec，V1 design baseline 直接列出最常用的 enum 值：

- `actor_type`: `framework_runtime | agent | human | observer | external_tool`
- `capture_mode`: `auto | agent_reported | human_reported | inferred`
- `confidence`: `high | medium | low`
- `trace_layer`: `workflow | agent_action | tool | human | evaluation`
- `telemetry_event.status`: `started | succeeded | failed | blocked | skipped | cancelled`
- `evidence.status`: `available | partial | missing | archived`
- `evaluation.result`: `passed | failed | warning | not_applicable`
- `evaluation.status`: `pending | passed | failed | waived`
- `violation.status`: `open | triaged | accepted | fixed | dismissed`
- `violation.risk_level`: `low | medium | high | critical`
- `artifact.status`: `draft | generated | reviewed | published | archived`
- `artifact.artifact_type`: `report | snapshot | bundle | attachment | deliverable`
- `artifact.artifact_role`: `audit | evaluation | violation_summary | improvement_proposal | deliverable | debug_attachment`
- `artifact.storage_scope`: `project_committable | project_local | exportable`
...
coverage evaluator 的最低证据闭包口径也要在设计基线里固定：

| control point | primary writer | minimum evidence closure |
| --- | --- | --- |
| `session_created` | runtime/dispatcher facade | `event` |
| `workflow_run_started` / `workflow_run_ended` | runner | `event` |
| `workflow_step_transitioned` | dispatcher / state-machine layer | `event` |
| `command_completed` | tool/runtime hook | `event + stdout/stderr evidence` |
| `patch_applied` / `file_written` | tool/runtime hook | `event + diff/file evidence` |
| `test_result_recorded` | tool/evaluation hook | `event + test result evidence` |
| `gate_hit` / `gate_blocked` | runner/dispatcher gate hook | `event + gate reason evidence` |
| `audit_report_generated` | governance publisher | `event + artifact ref` |
...
implementation plan 还必须写明禁止越界写：

- runtime instrumentation 不得直接写 snapshot 或 artifact current state
- evaluator / detector / generator 不得绕过统一 writer API
- CLI surface 不得直接修改 governance object current snapshot

## Manual Recording Surface To Freeze In Plan

V1 的 manual recording surface 不得只停留在“手工补 event”。implementation plan 至少要冻结以下最小能力：

- `record-event`
- `record-evidence` 或 `attach-note`
- `open-session` / `close-session` 作为非自动 runtime 场景的兜底入口
...
同时要配套 source closure 的正反对照：

- 来源闭包完整时 artifact 可以进入 `published`
- 已发布 artifact 的来源闭包失效时必须降级为 `reviewed` 或 `draft`
...
- telemetry 不存在时可以报告 `not_initialized`，但不得隐式创建 telemetry 根
...
- implementation plan 可以细化 task，但不得扩大 V1 scope，也不得删除 Frozen V1 Contract 中的约束
