...
| P3 | 跨平台首次用户 12 路完整闭环 | **WI222 formal 已完成；WI223 R02 最小证明载体 formal 已获批准** | 9.5/10 | WI223 目标 1.5–2.5 人日、硬上限 3 人日；完整实现仍估 6–10 人日 | 先验证单路复用 | P2 默认入口稳定 |
...
**状态：WI222 已完成 P3-A formal/admission；用户已批准 WI223 只实现 R02 最小证明载体。** 基于
...
执行器复用 guide replay 与 release smoke，生成临时 R02 receipt。不得修改 runtime、installer、release/version，
也不得把 PR 或手工 dispatch 冒充正式 release event。
...
- [ ] WI223 以一个共享执行器完成 R02 asset digest、已有项目接入、主动恢复和临时 receipt；真实 release event 前保持 `partial`。
...
- [ ] P3 十二条首次用户路线完成真实 clean-environment E2E；WI222 formal 已完成，WI223 R02 最小证明载体 formal 已获批准。
