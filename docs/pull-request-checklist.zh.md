...
- [ ] 若本 PR 涉及 release / 安装包 / 版本号：已同步核对 `README.md`、`docs/releases/v0.9.2.md`、`USER_GUIDE.zh-CN.md`、`packaging/offline/README.md` 与 release 资产文案，确保版本入口、平台资产分工、普通用户主路径和排障话术一致（当前 published release `v0.9.2`：`ai-sdlc-offline-0.9.2-windows-amd64.zip`、`ai-sdlc-offline-0.9.2-macos-arm64.tar.gz`、`ai-sdlc-offline-0.9.2-linux-amd64.tar.gz`）。不得只同步版本号/资产名而保留旧的 venv/pip、`adapter status` 主路径、`materialized only` 等历史话术。
...
- [ ] **本地 PR review**：若本 PR 使用 `ai-sdlc pr-review`，模型调用必须由开发机上的本地独立 review agent 发起，默认使用用户当前配置模型，也可显式选择已连接模型；显式模型不可用时必须 fail-closed 并说明不可用原因，不得静默 fallback。review 输入必须通过 `DiffSource` / source adapter 进入 review pack，本地仓、patch、staged/unstaged、GitHub/GitLab/Gitee/self-hosted SCM、公司内网 SCM 都不得写死为单一 GitHub PR 流程。关闭 review 后可用 `ai-sdlc pr-review attest` 写入 `latest-attestation.json`；CI 只读取 review artifacts、attestation、schema、commit hash、unresolved counts 和 final report，CI 不得发起模型请求，也不得替代本地对抗 review agent。
...
- [ ] **Loop Next Action Guidance**：若本 PR 修改 `loop status/list` 的下一步提示，必须确认 Next Action Guidance 只是基于本地 artifact 的只读推导；不得执行下一步命令，不得发起模型请求，不得把 guidance 包装成本地独立 review agent、CI gate 或人工确认的替代品。
