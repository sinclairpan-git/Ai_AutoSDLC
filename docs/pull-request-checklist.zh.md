...
- [ ] 若本 PR 涉及 release / 安装包 / 版本号：已同步核对 `README.md`、`docs/releases/v0.7.14.md`、`USER_GUIDE.zh-CN.md`、`packaging/offline/README.md` 与 release 资产文案，确保版本入口、平台资产分工、普通用户主路径和排障话术一致（当前 published release `v0.7.14`：`ai-sdlc-offline-0.7.14-windows-amd64.zip`、`ai-sdlc-offline-0.7.14-macos-arm64.tar.gz`、`ai-sdlc-offline-0.7.14-linux-amd64.tar.gz`）。不得只同步版本号/资产名而保留旧的 venv/pip、`adapter status` 主路径、`materialized only` 等历史话术。
