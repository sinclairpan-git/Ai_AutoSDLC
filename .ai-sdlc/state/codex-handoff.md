...
- State: branch=`feature/217-programservice-artifact-loader-dedupe-docs`，base=`b4d2ce5a5bc27b72549dcdf394f277cfbd6a124d`；PR #167 已推送旧 HEAD `1a24ace02fa448c152e11e082ba2a835c99d423b`，其评审与 CI 因终止合同变更失效；终止 source 与 R2 remediation 已提交为当前 committed+clean identity，等待双审，implementation blocked
- Stage: plan
...
- `program-manifest.yaml` 与 program truth 机械状态将在 source 双审通过后重新同步
...
- 当前 program truth 在 mapped authoring 变更后为 stale；当前 committed+clean source 双审通过后再重新 truth sync 与完整门禁。
...
1. 对当前 committed+clean HEAD/tree/formal-six取得 LEAN/SAFETY 同一身份 PASS0；finding只在本分支最小修复后重审。
2. 双审通过后重新 program truth sync，执行 manifest/constraints/validate/truth/scope/parity 门禁并取得最终同身份 PASS0。
3. 推送并更新 PR #167，只接受新 HEAD 的 review 与 required checks；全绿后合并并 detached fresh-main 验收。
4. 至多一次 implementation 尝试；随后一个 closure PR 无条件关闭 WI217/WI196、退役 RC-08、把剩余债务转 backlog并恢复特性开发；post-merge NO-GO由该closure同时回退。
