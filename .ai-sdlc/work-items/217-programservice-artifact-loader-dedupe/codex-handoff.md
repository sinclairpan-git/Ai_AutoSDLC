...
- WI217 canonical five docs；WI196 five docs 最小 active-child 更新
...
- 唯一方案：一个private helper +12 direct exact label bindings + cleanup-only wrapper；禁止模块、
  registry/reflection/DSL/getattr/type erasure、第二family和全文件formatter。
...
- 当前formal truth snapshot尚未基于最终source commit机械同步；在ready/fresh前不得评审或推PR。
