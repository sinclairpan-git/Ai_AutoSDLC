...
- 这份手册现在默认以**当前发布版 `v0.7.11`** 为准。普通用户优先使用 Release 离线包或公司管理员分发的安装包；只有框架开发者验证源码改动时，才使用本地源码安装。
...
pip install "https://github.com/sinclairpan-git/Ai_AutoSDLC/archive/refs/tags/v0.7.11.zip"
...
python -m pip install -U pip
pip install "https://github.com/sinclairpan-git/Ai_AutoSDLC/archive/refs/tags/v0.7.11.tar.gz"
python -m ai_sdlc --help
```
...
- Windows x64：`ai-sdlc-offline-0.7.11-windows-amd64.zip`
- macOS Apple Silicon：`ai-sdlc-offline-0.7.11-macos-arm64.tar.gz`
- Linux x64：`ai-sdlc-offline-0.7.11-linux-amd64.tar.gz`
...
在安装脚本提示可用的终端里执行；如果你的安装方式要求先激活 `.venv`，就先按安装脚本输出激活：

```bash
ai-sdlc self-update check
```

如果你现在的旧版本提示 `No such command 'install'`、提示“当前安装渠道未确认可自动升级”，或者执行更新后版本仍没有变化，说明本机 CLI 太旧，不能靠旧 CLI 自己完成升级。不要继续尝试 `pip` 或不存在的子命令，直接下载最新安装包，并让安装包覆盖当前 `PATH` 命中的旧入口。成功后，未来更新再回到 `ai-sdlc self-update check`。
...
`v0.7.11` 起，普通交互命令前出现的“检测到新版”提醒只会做短时间节流，不会因为显示过一次就永久当作用户已确认。也就是说，如果你错过第一次提醒，后续仍会再次看到更新提示；真正升级仍然执行 `ai-sdlc self-update check`。
...
- 如果输出包含 `Update check failed` 或“无法刷新 update state”，说明当前安装没有变化；旧版 CLI 请回到第零章下载最新 Release 离线包并执行 `--upgrade-existing`。
