...
```text
https://github.com/sinclairpan-git/Ai_AutoSDLC/releases/tag/v0.8.10
```
...
$PackageName = "ai-sdlc-offline-0.8.10-windows-amd64.zip"
$BundleName = "ai-sdlc-offline-0.8.10-windows-amd64"
...
install_ai_sdlc_offline() {
  PackageName="ai-sdlc-offline-0.8.10-macos-arm64.tar.gz"
  PackageDir="$(pwd)"
  ...
  tar xzf "$PackageName"
  cd ai-sdlc-offline-0.8.10-macos-arm64
...
# This assumes you have cd'ed into the directory containing the tar.gz. The script checks only the current directory.
install_ai_sdlc_offline() {
  PackageName="ai-sdlc-offline-0.8.10-linux-amd64.tar.gz"
  PackageDir="$(pwd)"
  if [ ! -f "$PackageName" ]; then
    echo "当前目录没有找到安装包：$PackageDir/$PackageName"
    echo "Package not found in the current directory: $PackageDir/$PackageName"
  ...
  tar xzf "$PackageName"
  cd ai-sdlc-offline-0.8.10-linux-amd64
  chmod +x install_offline.sh
  ./install_offline.sh --add-to-path
  ai-sdlc --help
}
...
$PackageDir = (Get-Location).Path
$ExtractRoot = Join-Path $PackageDir ".ai-sdlc-install"
Invoke-WebRequest -Uri "https://github.com/sinclairpan-git/Ai_AutoSDLC/releases/download/v0.8.10/$PackageName" -OutFile (Join-Path $PackageDir $PackageName)
New-Item -ItemType Directory -Path $ExtractRoot -Force | Out-Null
Write-Host "正在解压安装包；如果已经解压过，会安静覆盖同名文件。"
Write-Host "Extracting package; existing installer files will be overwritten quietly."
...
```powershell
Set-Location ..; $BundleName = "ai-sdlc-offline-0.8.10-windows-amd64"; $PackageName = "$BundleName.zip"; $PackageDir = (Get-Location).Path; $ExtractRoot = Join-Path $PackageDir ".ai-sdlc-install"; Invoke-WebRequest -Uri "https://github.com/sinclairpan-git/Ai_AutoSDLC/releases/download/v0.8.10/$PackageName" -OutFile (Join-Path $PackageDir $PackageName); New-Item -ItemType Directory -Path $ExtractRoot -Force | Out-Null; Expand-Archive -LiteralPath (Join-Path $PackageDir $PackageName) -DestinationPath $ExtractRoot -Force; Set-Location (Join-Path $ExtractRoot $BundleName); powershell -ExecutionPolicy Bypass -File .\install_offline.ps1 -AddToPath; .\.venv\Scripts\python.exe -m ai_sdlc --help
```
...
# 回到项目父目录，让安装包目录和业务项目目录同级
cd ..
curl -L -o ai-sdlc-offline-0.8.10-macos-arm64.tar.gz "https://github.com/sinclairpan-git/Ai_AutoSDLC/releases/download/v0.8.10/ai-sdlc-offline-0.8.10-macos-arm64.tar.gz"
tar xzf ai-sdlc-offline-0.8.10-macos-arm64.tar.gz
cd ai-sdlc-offline-0.8.10-macos-arm64
chmod +x install_offline.sh
./install_offline.sh --add-to-path
ai-sdlc --help
...
# 回到项目父目录，让安装包目录和业务项目目录同级
cd ..
curl -L -o ai-sdlc-offline-0.8.10-linux-amd64.tar.gz "https://github.com/sinclairpan-git/Ai_AutoSDLC/releases/download/v0.8.10/ai-sdlc-offline-0.8.10-linux-amd64.tar.gz"
tar xzf ai-sdlc-offline-0.8.10-linux-amd64.tar.gz
cd ai-sdlc-offline-0.8.10-linux-amd64
chmod +x install_offline.sh
./install_offline.sh --add-to-path
ai-sdlc --help
...
& "D:\work\.ai-sdlc-install\ai-sdlc-offline-0.8.10-windows-amd64\.venv\Scripts\ai-sdlc.exe" init . --agent-target codex --shell powershell
```
...
已有项目也优先使用 `v0.8.10` Release 包或公司安装包。这里也分两种场景：已提前下载离线包，或在线从 Release 下载并安装。
...
- 输出包含更新完成或安装完成的信息
- 输出包含当前安装版本，例如 `Installed version: 0.8.10`
...
- `self-update check` 没有把版本更新到 `0.8.10`
...
| Git Bash 显示旧路径下的 `ai-sdlc: No such file or directory` | 按本手册安装 `v0.8.10` 或更新版本并带 `-AddToPath`，安装完成后新开 Git Bash 再执行 `ai-sdlc --version` |
| `No module named ai_sdlc` | 当前 Python 环境没有安装 AI-SDLC；回到安装包目录重新安装 |
| `No such command 'install'` | 当前命令不支持自动升级；下载 `v0.8.10` 同平台包并执行 `--upgrade-existing` / `-UpgradeExisting` |
