...
`v0.9.4` is the current published framework release. This release refreshes the
Vue3 `public-primevue` frontend governance line to the v1.6.1 AI-executable
enterprise standard: PrimeVue theme tokens, route meta contracts,
`ApiResponse<T>`, TypeScript unknown-first boundaries, warning-only evidence,
Base / Business / View layering, and default collaboration tooling are now
aligned across docs, provider manifests, generation governance, CLI output,
adapter templates, and tests.
...
If you want the published release, install `v0.9.4`. If you are evaluating newer unreleased behavior beyond this tag, prefer the source-checkout path below.

- Current release notes: `docs/releases/v0.9.4.md`
