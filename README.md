...
`v0.8.6` is the current published framework release. This patch keeps the
Vue3 `public-primevue` default-provider behavior introduced in `v0.8.4`, while
making Windows upgrade and first-run output friendlier for ordinary users:
transient `project-config.yaml` metadata locks no longer stop dry-run, default
CLI output hides internal adapter proof terminology, and update guidance points
to the right installer path. Explicit `vue2` / `enterprise-vue2` selection for
built-in enterprise component-library users remains supported.

If you want the published release, install `v0.8.6`. If you are evaluating newer unreleased behavior beyond this tag, prefer the source-checkout path below.

- Current release notes: `docs/releases/v0.8.6.md`
- Windows offline bundle: `ai-sdlc-offline-0.8.6-windows-amd64.zip`
- macOS offline bundle: `ai-sdlc-offline-0.8.6-macos-arm64.tar.gz`
- Linux offline bundle: `ai-sdlc-offline-0.8.6-linux-amd64.tar.gz`
