...
after squash merges or when a local default branch is behind its remote.

New work-item specifications also prompt for observable value, evidence, a
minimal alternative, total delivery cost, and an exit condition before coding.
These are lightweight planning guidance, not a new command, state machine, or
blocker for existing specifications.
