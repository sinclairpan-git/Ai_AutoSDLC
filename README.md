...
`v0.8.5` is the current published framework release. This patch keeps the
Vue3 `public-primevue` default-provider behavior introduced in `v0.8.4`, while
making adapter verification and upgrade guidance clearer for ordinary users.
Explicit `vue2` / `enterprise-vue2` selection for built-in enterprise
component-library users remains supported.

If you want the published release, install `v0.8.5`. If you are evaluating newer unreleased behavior beyond this tag, prefer the source-checkout path below.

- Current release notes: `docs/releases/v0.8.5.md`
- Windows offline bundle: `ai-sdlc-offline-0.8.5-windows-amd64.zip`
- macOS offline bundle: `ai-sdlc-offline-0.8.5-macos-arm64.tar.gz`
- Linux offline bundle: `ai-sdlc-offline-0.8.5-linux-amd64.tar.gz`
