...
4. 重复算法族从 216 LOC 降为 12 LOC，下降 94.4%；包含新文件与 18 条 import 的 raw replacement
   slice 从 216 降为 33 LOC，下降 84.7%。
5. 固定 19 个产品文件 raw diff 为 additions≤33、deletions≥216、net≤-183；定义数 18→1，
...
算法、签名、annotation、调用表达式和异常时序不引入分支。预测产品 +33/-216/net -183，满足
LP-02、RC-04、RC-06。
...
2. **Given** 产品 additions≤33、共享身份 test≤4、root truth replacement additions=2，
   **When** 计算最终 ledger，**Then** 总计≤39、余量≥15；文档不冒充代码预算。
...
- **RC-03 预算**：新增私有模块=1、产品 raw additions≤33；新测试文件/fixture/harness/normalizer=0；
  共享身份 test≤4；root truth additions=2。
- **RC-04 结果**：重复族 18→1；算法 216→12（-94.4%）；raw replacement 216→33（-84.7%）；
  product net≤-183；complexity 72→4；纯移动、wrapper 或第二 helper 均 No-Go。
- **RC-05 临时膨胀**：implementation commit 同时加入 helper/import 并删除 local body；不得长期保留
  19 份实现。任一 commit 产品 additions>33 即停止。
- **RC-06 保护成本**：删除 216 对应 cap 54；最终 source additions≤39，余量≥15；删除不抵扣新增。
...
- **SC-04**：最终 defs=1、imports=18、calls=100、body/FunctionDef/signature/binding 合同通过，无 wrapper、
  public export 或新依赖。
- **SC-05**：产品 +≤33/-≥216/net≤-183；全部 source additions≤39≤54。
