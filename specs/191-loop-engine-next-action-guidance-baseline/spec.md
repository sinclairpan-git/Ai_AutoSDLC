...
5. `loop list --json` 的每个 item 带独立 guidance，便于工具显示历史 run 的可恢复动作。
...
**独立测试**：执行 `loop list --json`，断言顶层 result 和每个 item 都包含 `next_guidance`，且字段为稳定 snake_case。

**验收场景**：

1. **Given** `loop status --json` 输出 ready，**When** 工具解析 JSON，**Then** 顶层必须包含 `next_guidance`，当前 loop 也必须包含自己的 `next_guidance`。
2. **Given** `loop list --json` 包含多个历史 run，**When** 工具读取 items，**Then** 每个 item 必须包含 `next_guidance.command`、`reason`、`requires_model`、`writes_artifacts`、`writes_code`、`evidence`。
...
作为框架维护者或高级用户，我希望 `loop list` 中每个历史 run 都能显示自己的下一步，以便快速判断哪个 run 可关闭、哪个 run 需要修复、哪个 run 损坏。

**优先级说明**：P0 先保证当前 status，P1 扩展到 list item，便于审计和工具集成。

**独立测试**：构造一个 `needs_fix` run 和一个 `passed` run，执行 `loop list --json`，断言每个 item 的 guidance 与自身 status 匹配。

**验收场景**：

1. **Given** 历史 run 状态不同，**When** 执行 `ai-sdlc loop list`，**Then** 每个 run 的 human 摘要必须显示对应 `Next command`。
...
- **SC-191-003**：`loop list --json` 的每个合法 item 都包含结构化 `next_guidance`，且不移除原 `next_action` 字段。
