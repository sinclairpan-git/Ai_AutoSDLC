...
- **FR-191-005B**：`loop list --json` 遇到 malformed current pointer 或 missing current target 时，即使存在可读历史 run，顶层 `next_guidance` 也必须是 blocked repair guidance，并把 pointer/target path 放入 evidence。
