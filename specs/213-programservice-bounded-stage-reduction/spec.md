...
- 生成版本化 Python-surface manifest：27 个 public callable 的 `inspect.signature`、parameter kind/default/
  ...
  禁止修改的 DTO `__post_init__` 比较 presence/module/qualname/signature/source-or-normalized-code SHA-256 +
  行为 digest；default/default_factory 使用 MISSING、typed literal、source-backed callable fingerprint，或仅
