...
- 生成版本化 Python-surface manifest：27 个 public callable 的 `inspect.signature`、parameter kind/default/
  ...
  `dataclasses.fields` name/type/default/default_factory/order 及 `__post_init__` 可移植 fingerprint。Callable
  fingerprint 固定为 presence/module/qualname/signature/source SHA-256 与代表性行为 digest；default/
  default_factory 使用 MISSING、typed literal 或 callable module/qualname/source hash 编码。源码不可读取则
  fail-closed；禁止 `id()`、对象 identity 或含内存地址的 `repr()`；
...
- **SC-005**：formal 关闭只授权创建一个 T58/GAP-15 WI；T58 fresh-main 后才授权唯一 T66
