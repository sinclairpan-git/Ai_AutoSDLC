...
Delivery PR #163 已 merge 为 `60fe6d90`；closure receipt PR #164 reviewed HEAD/tree=`428a316a`/
`cc3c6b7f`，LEAN/SAFETY 同身份 PASS0、Codex clean、required checks 全绿并 merge=`7922956d`，detached
fresh-main 全绿。GAP-15/T58 已关闭；唯一 implementation WI215 已创建，当前停在 T61A 双 readiness 前，
产品代码保持零差异。
...
2. 每个stage以单一test-only runner在隔离T61A原始legacy、candidate checkout current legacy、candidate
   route三方完成differential；未轮到的stage继续legacy。
...
| RC-06 | 新增 product≤522，新增 test/harness/normalizer≤290，组合硬门≤`floor(2918×25%)=729`；个别上限不得相加使用；既有保守 subtotal184，路线累计≤913≤1,500 |
...
T61A产品尚未编码时不得用actual product=0绕过RC-06：门禁必须使用`candidate_product_shadow +
actual_current_proof + reserved_future_proof≤729`，future reserve逐文件/任务冻结且不得记0规避；candidate
terminal改用`actual_peak_product + actual_total_proof≤729`。当前implementation WI按逐文件/symbol可执行
runner任务冻结reserve=90；实际超出时须等量降低product shadow，否则NO-GO。
...
- 固定 formal merge commit/tree、Python/OS/dependency、九阶段45-symbol map和LOC/branch recipe；
- 重跑exact165 ordered nodes与两个独立basetemp，冻结目标test、实际fixture/helper/config blob及命令结果；
- warm-up后至少20次selector原始duration记录p50/p95；T61B三方同机重采；
- 生成版本化 Python-surface manifest：27 个 public callable 的 `inspect.signature`、parameter kind/default/
  annotation、return annotation、`__annotations__`、`__doc__`；九组DTO的module/qualname、fields顺序/type/
  default/default_factory/equality及`__post_init__` presence/signature/source SHA；不重复调用行为；
- 唯一recorder目标≤170/hard cap200，只生成JSON-primitive五section原子receipt/verify；不引入typed canonicalizer、
  normalizer或逐node transitive AST mapper；
- 预测architecture与proof LOC；product>522、proof>290、product+proof>729或需要非§4.1结构即readiness `NO-GO`；
- Pascal/LEAN 与 Confucius/SAFETY 对同一 T61A identity 均明确 `GO` 后才可写产品代码。

### T61B：candidate pre-merge differential

- 每stage/T61B必须由既有测试文件中的唯一test-only runner三方运行：隔离T61A原始legacy commit/tree、
  candidate checkout current legacy、candidate；不得新增脚本/测试文件/receipt schema；
- 三腿必须使用各自`uv run --isolated --project <leg-root>`环境，并在矩阵前验真cwd、HEAD/tree、解释器、
  resolved module file/source SHA与route marker；leaf固定`-o pythonpath=<leg-root>/src --import-mode=importlib`，
  共享或指错editable/candidate pytest source立即fail-closed；
- 三方使用byte-identical seed，逐方法与完整链比较loader/builder、request/result、exception、transcript、
  raw artifact/report、tree/mode/write order、late-bound、fault/termination/retry、transient sentinel与副作用；
- 对T61A Python-surface manifest逐字段零差异；candidate engine 必须
  接收从 `self` 解析的 public callbacks，不得旁路 override/monkeypatch；
- 三腿复用同一重置后的绝对behavior root，绑定`comparison_policy=same-absolute-root-v1`且不使用normalizer；
  同时绑定candidate commit/tree、LOC/branch/runtime ledger，零未批准差异；
- selector legacy→candidate→legacy→candidate 四个 receipt 均实际运行；
- 执行与采集优先精确复用T61A冻结的106 unit+59 integration节点及九个既有stage seed helper；implementation
  WI按anchored primary-stage grammar冻结九个disjoint exact-node组，union必须精确106+59；三腿worker直接
  选择candidate测试定义中的同一exact IDs，record/verify/worker分组前均拒绝thread_archive/project_cleanup
  node，JUnit逐项对账。两个测试文件各九个helper须file-qualified并绑定
  source SHA；只对未输出的fault/termination/sentinel/network ledger和三方性能增加逐symbol最小capture；
- targeted、full、Ruff、constraints、truth 与两个本地 reviewer current-identity PASS 后才允许 candidate PR。

Deletion current-head/fresh-main三腿改为T61A原始legacy、冻结pre-deletion candidate-merge current legacy、
deletion candidate；删除后的checkout不承担已不存在的current legacy腿。三腿仍须完整执行上述矩阵。
...
- **SC-002**：45 methods=`3,638/3,305/386`、165 tests=`106+59` 可复算，预算 `product≤522/proof≤290/combined≤729/terminal≤720/net delete≥2,918`
