...
**状态**：PR compatibility remediation 已通过本地全量；待新 evidence-freeze 与 replacement sync
...
- PR synthetic merge commit 证明 capability `ready` 是上下文状态；测试已只保留稳定 `closed` invariant，clean checkout 的 `ready/[]` 继续由 truth audit 与 merge smoke 验证。
- compatibility candidate targeted 为 `1 passed in 56.45s`，full 为 `3186 passed, 3 skipped in 480.66s`；Ruff、validate、constraints、dry-run 全部通过。

## 待完成

- compatibility evidence-freeze、replacement sync、rollback drill、双终审、PR 重审/CI 与 mainline smoke 尚待执行。
