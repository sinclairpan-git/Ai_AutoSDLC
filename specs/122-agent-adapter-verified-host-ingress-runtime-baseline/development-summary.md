# 开发总结：122-agent-adapter-verified-host-ingress-runtime-baseline
...
- 闭环口径：adapter runtime 已从 `installed / acknowledged / activated` 的旧治理表述迁移到 ingress truth。；`adapter activate` 仍可保留为兼容面，但不再决定 `run` 是否放行。
