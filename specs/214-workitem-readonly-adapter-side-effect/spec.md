...
| `init` hook exception | 1 | preflight 后异常原样传播；scaffold 不开始 |
...
| `link` hook exception | 1 | handler 不开始；checkpoint 不写 |
...
- **FR-002**：五个只读命令在真实写入型 sentinel hook 下必须保持 adapter/config/working-tree bytes
  不变且不出现 install receipt。
- **FR-003**：五个只读 handler 的业务输出、错误输出和退出码必须与同 revision no-op-hook 基线逐字节一致。
- **FR-004**：`init/link` 必须满足 §3.2 全矩阵，且不得通过改测试、吞异常或延后断言伪造兼容。
...
- **FR-008**：formal 使用 docs branch/PR；实现从 formal fresh main 建 dev branch/PR；每个 PR 可独立 revert。
- **FR-009**：实现 PR 必须完成 Codex current-head review、required checks、merge 与 detached fresh-main。
- **FR-010**：T58 fresh-main 前不得创建 T66 implementation WI/T61A；RC-08 前不得发布版本。

## 5. 成功标准

- **SC-001**：`5 × 3 = 15` 个只读 CLI 入口全部 hook count=`0`、sentinel bytes/clean-tree 不变。
...
- **SC-006**：formal 和实现 current identity 均取得双 Agent PASS0；实现 PR Codex/CI/merge/fresh-main 有可追溯 receipt。
- **SC-007**：parent GAP-15/T58 在 fresh-main 后才标记关闭；T66 T61A 仅在该关闭事实后恢复。

## 6. 停止、回退与发布边界

若实现需要修改 adapter 算法、root callback、handler、第二 CLI family，或无法保持 §3.2 零差异，立即停止并
回到 formal，不用扩范围掩盖问题。Formal PR 可独立 revert；实现失败时 revert 实现 PR 即恢复 legacy
分发，T66 继续阻断。WI214 局部完成不触发 tag、GitHub Release、PyPI 或共享 CLI 更新。
