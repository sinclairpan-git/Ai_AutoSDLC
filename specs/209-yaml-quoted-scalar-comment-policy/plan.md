...
PR #146 的 Round 12 双审与 Codex review 后，Windows full CI 证明 mixed-path 测试夹具物化了非法
Windows 文件名。Round 13 只修现有测试：真实 Git 使用跨平台合法路径，mixed raw-Unicode/C-escape
直接验证 decoder；产品实现和预算边界不变。初次 normalized 复算因 direct assert 被 Ruff 展开而得到
测试 `+201`；仅缩短 witness 的非语义文件名后，全部 escape 场景不变，最终 raw 产品/测试
`+121/+200`、normalized `+128/+198`。Round 13/14 的 lifecycle findings 退役后，Round 15 同一身份
Pascal/Confucius 双 PASS，Codex 审到 `c5c6e94adc` 且 22/22 checks 全绿；PR 合并为 `31aad572`。
fresh detached main 的 focused `100 passed`、full `3275 passed, 3 skipped`、Ruff/constraints/validate/
truth/manifest/clean guard 全部通过，GAP-14/T57 已关闭。
...
uv run ruff format --check src/ai_sdlc/core/comment_policy.py tests/unit/test_comment_policy.py tests/integration/test_cli_verify_constraints.py # 候选临时副本
# 在 formal-base 临时副本重复同一命令；退出码与 would-reformat 文件集合必须一致。
...
4. 已在 merge `31aad572` 的 fresh detached main 复跑 focused/full/Ruff/constraints/truth/clean guard；
   结果通过并关闭 GAP-14/T57。
...
- Ruff lint 必须 exit 0；若 formal base 存在 formatter 债，base/candidate disposable copies 的 format
  check 退出码与 `Would reformat` 文件集合必须完全一致，不把 inherited 非零写成 format PASS。
