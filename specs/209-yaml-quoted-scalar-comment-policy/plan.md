...
raw-Unicode/C-escape 直接验证 decoder；产品实现和预算边界不变。初次 normalized 复算因该 direct
assert 被 Ruff 展开而得到测试 `+201`；仅缩短 witness 的非语义文件名后，全部 escape 场景不变，最终
raw 产品/测试 `+121/+200`、normalized `+128/+198`。focused/full 与 pre-receipt truth/manifest 已通过，
当前重新绑定 final truth freeze/replay/双审/current-head review/checks。
