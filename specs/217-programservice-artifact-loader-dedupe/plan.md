...
| `specs/217-programservice-artifact-loader-dedupe/*` | candidate 合同、任务、账本、结论 |
...
- Create: `specs/217-programservice-artifact-loader-dedupe/{spec.md,plan.md,tasks.md,task-execution-log.md,development-summary.md}`
...
- [ ] **Step 1: 写完 canonical formal 五件套并同步 parent 最小状态**
...
- [ ] **Step 2: 注册 manifest 与 sequence**
  ...
  Expected: inventory=`1136/1136`、close layer=`216/216`、missing/unmapped=`0/0`。
...
- [ ] **Step 3: formal gates**
  ...
  Expected: manifest test `1 passed`；constraints 无 BLOCKER；validate PASS；truth ready/fresh；diff-check
  exit0；formal diff 不含 `src/**` 或产品测试逻辑。
...
**Files:** evidence-only updates to WI196/WI217, manifest/truth and handoff after product identity is frozen.
...
- [ ] **Step 2: 运行全量与治理**
  ...
  Expected: pytest exit0；无 BLOCKER；validate PASS；truth ready/fresh、missing/unmapped=0。
...
- [ ] **Step 1: committed+clean same-identity 双审**
  ...
  LEAN 与 SAFETY 同时审查 candidate commit/tree、final HEAD/tree、formal-six、`+96/-406` code/test diff、
  truth 与 rollback receipt。任何 tracked 内容变化使两者同时失效。

- [ ] **Step 2: implementation PR**

  推送 implementation branch，创建 PR，请求 Codex review 并保持约五分钟 heartbeat；若 Codex 服务不可用，
  仅在用户已经授权的本地 SDLC 双审替代范围内继续，required CI 不得 waiver。

- [ ] **Step 3: merge 与 detached fresh-main acceptance**

  current-head 无 actionable finding、required checks 全绿且 LEAN/SAFETY PASS0 后 squash merge，不删除本地
  branch。detached fresh-main 重跑 Task 3 Step3 与 Task 4 全部验证，并证明 merge tree 包含 exact reviewed
  product/test blobs。

- [ ] **Step 4: closure receipt**

  独立 records-only PR 只关闭本 T63 family并登记实际 product net -358；保持 GAP-03/T66、GAP-05、WI196、
  RC-08、release open。closure fresh-main 通过后才选择下一原子 gap/reduction work item。
