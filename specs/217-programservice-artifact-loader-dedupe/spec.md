...
本工作项修复一个 T63 精确重复族并为 RC-08 提供真实产品净删除，但不关闭 GAP-03/T66、GAP-05、
WI-196、RC-08 或 release。它不继承 WI-213/WI-215 的结构、预算、GO、hash 或 reviewer receipt。
...
- 仓库代码与测试中没有这 13 个私有名称的消费者；历史 specs 只作记录，不是运行时 seam。
...
禁止新增模块、public symbol、依赖、配置、环境变量、registry、reflection、DSL、`getattr`、stage-name
分支、dynamic dispatch、type erasure、fallback 或兼容 alias。禁止合并 payload builder、修改第二个重复族、
顺手格式化整个 17K 文件或改变注释语义。
...
1. 12 个 `ProgramService` caller 的 exact `artifact_label` source binding；
2. common helper 的 missing、invalid YAML（含 exact exception）、non-mapping、valid mapping 四态。

cleanup 的 invalid exact path/error 与 warning normalization 复用现有六组测试，不新增平行 harness。候选完成后
新增 proof 应为 16/16 GREEN，完整 ProgramService unit 应为 422/422 GREEN。T61B 还必须运行 CLI integration、
full suite、跨平台/安装包 smoke 与 disposable clone rollback/reapply。

## 6. 允许与禁止文件

### Formal PR

允许：WI217 五件套、WI196 五件套的最小状态追加、`program-manifest.yaml`、project-state、root/scoped
handoff，以及 manifest exact 两个机械计数。禁止 `src/**`、产品测试逻辑、workflow、依赖、版本、release。
...
另允许 WI196/WI217 records、manifest/truth、root/scoped handoff 和最多两行机械 inventory expectation。
禁止其他产品/测试文件。
...
- **FR-217-003**：missing/invalid/non-mapping/valid/cleanup warnings 的 observable 结果必须零未批准差异。
...
- **FR-217-007**：只关闭本 T63 family；GAP-03/T66、GAP-05、WI196、RC-08、release 保持 open。
...
- **SC-217-003**：16 个新增 proof、422 个 ProgramService unit、CLI integration、full suite、Ruff、治理、
  required CI、package/offline smoke 全绿。
- **SC-217-004**：disposable clone revert 恢复 exact baseline tree/406 tests，reapply 恢复 candidate tree/422 tests。
- **SC-217-005**：LEAN/SAFETY 对 final exact HEAD/tree/formal-six 均 PASS0/findings=0；PR checks 全绿并完成
  detached fresh-main 验收。
- **SC-217-006**：只把实际产品净删358计入 RC-08 family ledger，不关闭任何父路线总体目标或发布版本。

## 9. 回退

实现产品与 proof 作为一个原子 candidate commit；回退该 commit 同时恢复 13 个 legacy loader、12 个旧
callsite 和 406-test baseline。formal receipt 不强制 implementation GO；若 implementation NO-GO，只追加
失败 receipt 并保持本 family open，不删除或改写历史 formal。
