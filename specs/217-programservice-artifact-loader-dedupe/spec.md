...
和一个 records-only closure PR；implementation 的修复与重审必须留在同一分支/PR，不得以第二个 PR
重启候选。若在开 PR 前已确定 NO-GO，可以不创建 implementation PR；若 PR 已创建后确定 NO-GO，关闭
该 PR 且不合入产品，并直接进入唯一 closure PR。
...
Closure 不以 RC-08 达成为前提：GO 路径登记实际净删，NO-GO 路径登记零产品合入；两条路径都必须关闭
...
- **SC-217-006**：GO 时只登记实际产品净删；NO-GO 时明确零产品合入、不得伪造收益。两条路径的 closure
...
失败 receipt、关闭已开的单个 implementation PR且不合入产品，然后进入唯一 closure PR；不得保持路线
open、不得创建第二个 implementation 或新的减重 work item，也不删除或改写历史 formal。
