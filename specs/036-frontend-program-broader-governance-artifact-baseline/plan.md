...
---
# 实施计划：Frontend Program Broader Governance Artifact Baseline

**编号**：`036-frontend-program-broader-governance-artifact-baseline` | **日期**：2026-04-03 | **规格**：specs/036-frontend-program-broader-governance-artifact-baseline/spec.md

## 概述

本计划处理的是 `035` 之后的 frontend program broader governance artifact baseline，而不是继续扩张 `035` 的 broader governance responsibility。当前仓库已经在上游锁定并实现了：
...
- broader governance result 目前只存在于内存对象和临时终端/report 输出中
- final governance execution 若继续推进，将缺少稳定的 governance artifact 可供复用
- 若继续直接编码，容易把 governance artifact 与 final code rewrite / governance execution 混成过宽工单

因此，本计划的目标是建立统一的 `Frontend Program Broader Governance Artifact Baseline`：

- 先冻结 governance artifact 的 truth order、non-goals 与 write boundary
- 再冻结 artifact payload contract、source linkage 与 honest output boundary
- 最后给出后续 `core / cli / tests` 的推荐文件面与测试矩阵

当前 formal baseline 已完成。下一步允许进入首批实现切片：`ProgramService broader governance artifact writer`，随后再进入 CLI execute/write surface。当前仍不直接进入 final code rewrite / governance execution。

## 技术背景

**语言/版本**：Python 3.11+、Typer CLI、Markdown/YAML artifact  
**主要依赖**：`035` broader governance request/result、现有 `ProgramService` / `program_cmd.py`  
**主要约束**：

- governance artifact 只能消费既有 broader governance truth，不得另造第二套 governance result context
...
```text
specs/036-frontend-program-broader-governance-artifact-baseline/
├── spec.md
├── plan.md
├── tasks.md
...
src/ai_sdlc/
├── core/
│   └── program_service.py                   # current slice: broader governance artifact writer + loader boundary
└── cli/
...
- governance artifact 只消费 `035` broader governance request/result truth
- artifact 只能在显式确认后的 execute 路径写出，dry-run 不得默认落盘
- final code rewrite / governance execution 仍留在下游 work item
- 当前 baseline 不改写 `035` broader governance truth 或更上游 truth

未锁定上述决策前，不得进入 `036` 的代码实现。

## Owner Boundaries

| Owner Area | Responsibility | Forbidden Cross-Layer Writes |
| --- | --- | --- |
| governance artifact truth | 定义 artifact path、payload contract 与 write boundary | 不得改写 `035` broader governance truth |
| service writer | 定义 service 如何把 request/result materialize 为 artifact | 不得直接进入 final governance execution |
...
**目标**：将 frontend program broader governance artifact 从 downstream suggestion 收敛为单独 canonical formal docs。  
**产物**：`spec.md`、`plan.md`、`tasks.md`、`task-execution-log.md`。  
**验证方式**：文档对账 + `uv run ai-sdlc verify constraints`。  
**回退方式**：仅回退 `specs/036/...` 文档改动。

### Phase 1：Broader governance artifact truth freeze

**目标**：锁定 broader governance artifact 在 `035 -> 036 -> future final governance runtime` 主线中的 truth order。  
...
### Phase 4：ProgramService broader governance artifact writer slice

**目标**：在 `ProgramService` 中落下 canonical broader governance artifact writer。  
...
### Phase 5：Program broader governance artifact CLI surface slice

**目标**：把 broader governance artifact path/report 暴露到独立 CLI surface，并保持只在显式 execute 后写出。  
...
- 先在 `program_service.py` 定义 broader governance artifact writer / payload
