...
唯一 proof code：`scripts/program_bounded_stage_t61a.py`，目标≤150 physical LOC、硬上限160，
每函数≤50行。唯一机器生成基线：
...
RC-06 的 proof≤190 是全部新增手写 test/harness/normalizer 总和，不是 recorder 单文件额度。预算冻结为：
recorder≤160；manifest inventory assertion 机械替换按 diff 新增行计费；双 GO 后 candidate unit/CLI seam
与任何其他 test/helper/normalizer 的全部新增行共同使用剩余额度，最终总和≤190。现有测试/fixture 不重复
计费。Recorder 自然实现或后续最小 seam 无法留在总额内即 T61A/T61B NO-GO，禁止反射、DSL或晦涩压缩。
...
Recorder 对 canonicalization/matrix fault 以及父进程观察到的 isolated `SystemExit`/process termination
必须先在输出目录写临时文件、flush/fsync、`os.replace` 原子生成 `outcome=no_go` receipt，再以非零退出；
不得在同进程吞掉实际termination。原子持久化失败时保留原异常并非零退出，不能伪造closure。
...
Normalizer 每次 record/verify 只允许两个行为临时根；只可把这两个精确绝对前缀替换为 `<root-a>`/
`<root-b>`，以及把 clock spy 证明来自默认时钟的值替换为 `<clock>`；必须保留 clock 次数/顺序。
...
| RC-06 | proof≤190；product+proof≤712；既有测试/fixture 不重复计费 |
...
当前 architecture 预测为 private engine 348、binding/glue 84、candidate route/facade 64，peak product
496≤522；terminal=`225 header + 42 facade + 84 glue + 348 engine = 699≤720`，预计净删2,939。
...
- **FR-002**：proof harness 必须单文件≤160 LOC、无新依赖，并由机器生成唯一 receipt；全部 proof
  新增行≤190。
...
- **SC-001**：T61A proof identity 上 `src/**` 零差异、目标测试 blob 不变、manifest test 仅机械数字
  替换、总 proof≤190、165/165 通过、全部矩阵 PASS，双 readiness GO。
- **SC-002**：candidate peak product≤522、T61B zero-delta、full/governance/跨平台 checks 全绿，legacy 保留。
...
以下任一成立立即 `NO-GO`：45/3,638/3,305/386 或 165 库存无法解释漂移；proof>190；product>522；
需要第二模块/DTO迁移/CLI改动/新依赖/公共开关；surface无法 canonicalize；normalizer 越权；双根、异常、
中断、重试、side-effect 或 raw bytes 不稳定；任一 reviewer 非 GO。NO-GO 必须先保存不可变 receipt、
identity 和 verdict，然后停止产品路线，legacy 保持不变。Recorder 内部失败必须满足 §4 的 durable
