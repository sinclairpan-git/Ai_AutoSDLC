...
**类型**：WP-07 / GAP-04 / GAP-12 / GAP-13 / L3 candidate implementation
...
disposition 假绿；后续 RC-09 disposition PR 只可追加本规格 3.1 节冻结的 GAP-13 窄修，不得写
candidate handler/runner 产品代码或 T61A 保护代码。Formal
...
### 3.1 GAP-13：pre-close 恢复与零任务执行必须 fail closed

PR #130 的 Codex review 证明两个状态边界仍不完整：`development-summary.md` 存在会让 reconcile
仅凭文件把 `execute` 推到 `close`，而当前 branch disposition 仍为 `merge-pending`；同时 Runner
会在 `TasksParser` 返回零任务时仍构建 `Executor`，先改写 execution plan/runtime/working-set 再失败。
...
- 不新增模块、公共 API、schema 或抽象；candidate 9 个 handler、`program_cmd.py`、ProgramService、
  activation receipt 与已拒绝 T61A 代码均不得改动。当前结论是零 candidate 产品代码，不再表述为
  整个仓库零产品变化。
...
- **FR-00A pre-close 真值**：GAP-13 的 `close-pending` 使 status/recover 稳定停在 execute；零解析
  任务在构建 Executor 前无写入返回，close-check 仍按 lifecycle 终态阻断。
...
- **SC-00A**：close-pending 的 detect/recover/status 均不推进 close；无/未知 marker 兼容；零任务
  不构建 Executor、不写 runtime/plan/working-set；root/scoped ResumePack 在 fresh clone 连续加载后干净。
