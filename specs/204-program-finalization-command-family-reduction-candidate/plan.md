...
内先行修复，但不进入 candidate LOC/claim。
...
Formal admission 的 GAP-12 不是 candidate 产品实现。路径严格限于 spec 的 16 项白名单；
runtime/scaffold/rule/template 新增≤80 行、tests 新增≤180 行、合计≤260 行，且删除不能抵扣。
实现只增加显式状态 allowlist 和调用方的 private `_require_final_branch_disposition`
...
6. 运行 constraints、truth、path whitelist、diff check；除 GAP-12 窄修外，仅允许
   `test_repo_program_manifest.py` 两个既有 tuple 的 `1071→1076`、`203→204` 精确同步，不修改
   其他仓库真值测试逻辑。
7. PR 经 Codex review、required checks 后合入 main；本阶段不授权 candidate/T61A 产品或保护编码。
