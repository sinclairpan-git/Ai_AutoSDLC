...
本计划把已冻结 PRD 落成 AI-SDLC 的 P0 本地对抗 PR Review 能力。实现重点是：新增 `ai-sdlc pr-review` 命令组、统一 loop/review artifact 模型、review pack 生成、schema validation、policy profile、`mock-reviewer`、`codex-local` runner 合同、beginner-friendly `doctor` 和 close/check 集成。

P0 不实现 GitHub/GitLab inline comment、多 reviewer 投票、artifact 签名，也不接入 Codex 云端 PR review。CI 只能读取本地 artifact 或 attestation，不能调用 GPT/Codex 模型。
...
**约束**：不在 CI 中调用模型；不使用 Codex 云端 PR review；reviewer P0 不改代码。

## 宪章检查

| 宪章门禁 | 计划响应 |
|----------|----------|
| MVP 优先，范围严控 | P0 只做本地 diff pseudo PR、artifact、mock/codex-local 合同、doctor、schema/policy；远端 PR comment、多 reviewer、签名放 P1/P2。 |
...
2. `codex-local`：本地 reviewer runner 合同。P0 必须支持用户配置的本地命令或等价 launcher，输入为 review pack path，输出为 findings path；focused tests 必须覆盖“已配置本地命令可运行并生成合法 findings”和“未配置进入 `needs_user`”两条路径。
...
ai-sdlc pr-review start [--base <ref>] [--provider <provider>] [--dry-run] [--json]
...
4. `verify constraints` 覆盖 schema surface、云端 PR review 禁用文档/代码 surface、CI no-model 文档 surface。
...
```text
ai-sdlc pr-review doctor
ai-sdlc pr-review start --provider codex-local
```

若 policy profile 禁止外部模型，provider 启动必须被阻断，输出 plain-language blocker 和下一步命令。
...
| Policy blocks external provider | unit + CLI integration | doc surface check |
...
| Configured `codex-local` runner | unit + integration fixture command | unconfigured `needs_user` path |
...
| CI no-model boundary | verify constraints surface | workflow grep tests if workflows change |
...
| `codex-local` host 集成 | P0 做 configurable local command runner 合同；真实 Codex host 深集成另起 P1/P2。 |
