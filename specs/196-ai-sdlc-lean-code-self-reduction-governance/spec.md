...
| GAP-14 | 开放 | `comment_policy._is_comment_line()` 按 stripped diff line 判断前缀；YAML single-quoted scalar 的续行内容以 `#139...` 开头时被误判为被删除注释，导致 `verify constraints` 假 blocker | T57 / WI-209：T56 依赖已满足；下一独立 formal 先以 quoted-scalar diff characterization 固定误报，再做 path/syntax-aware 最小修复；不得用 execution-log waiver 掩盖 | 是（验证可靠性） |
