...
| GAP-15 | 关闭材料就绪（lifecycle fresh-main 前保持 active） | `main@e184c8e2`：`program validate` 前后 `.cursor/rules/ai-sdlc.mdc` SHA-256 均为 `d5f04acf...0b6a`；只读 `workitem plan-check --json` 却输出 `IDE adapter (cursor): installed 1 file(s)` 并将 SHA 改为 `02d9656d...e134`，产生 `+18/-6` tracked diff。根因是 `workitem` callback 对除 `init` 外全部子命令无条件调用 adapter hook | T58 / WI-214：formal PR #160、amendment PR #161、implementation PR #162；reviewed tree=`03b4a1ff`，本地 LEAN/SAFETY 同身份 PASS0，22/22 checks，merge=`2845fedc`；detached fresh-main full=`3303 passed, 3 skipped`，targeted=`50 passed`，truth=`ready/fresh 1126/1126`，全部门禁全绿；只隔离五个只读命令的隐式 refresh，保留 `init/link` 既有写语义；本 lifecycle fresh-main 后才关闭 | 否 |
...
11. WI-213 formal 验证新发现 GAP-15：`program validate` 已保持 adapter bytes，但 `workitem` callback 仍让
    ...
    PR #161 与 implementation PR #162 / merge `2845fedc` 已完成；本 lifecycle reconciliation
    merge/fresh-main 后关闭 GAP-15/T58。此后唯一下一步是创建 T66 implementation WI并先完成 T61A 双
    readiness，仍不得把缺陷修复计入 T66 减重收益。
