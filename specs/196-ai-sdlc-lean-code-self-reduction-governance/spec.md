...
| GAP-15 | 已关闭（本 lifecycle reconciliation merge/fresh-main 生效） | `main@e184c8e2`：`program validate` 前后 `.cursor/rules/ai-sdlc.mdc` SHA-256 均为 `d5f04acf...0b6a`；只读 `workitem plan-check --json` 却输出 `IDE adapter (cursor): installed 1 file(s)` 并将 SHA 改为 `02d9656d...e134`，产生 `+18/-6` tracked diff。根因是 `workitem` callback 对除 `init` 外全部子命令无条件调用 adapter hook | T58 / WI-214：formal PR #160、amendment PR #161、implementation PR #162；reviewed tree=`03b4a1ff`，本地 LEAN/SAFETY 同身份 PASS0，22/22 checks，merge=`2845fedc`；detached fresh-main full=`3303 passed, 3 skipped`，targeted=`50 passed`，truth=`ready/fresh 1126/1126`，全部门禁全绿；只隔离五个只读命令的隐式 refresh，保留 `init/link` 既有写语义 | 否 |
