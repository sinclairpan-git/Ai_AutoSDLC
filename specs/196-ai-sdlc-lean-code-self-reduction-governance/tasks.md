...
| T58 隔离 workitem 只读 adapter side effect（active） | standalone | GAP-15 | L2 / CC-05 | WI-213 PR #158 / merge `450d4988` 与 lifecycle PR #159 / merge `d5ad7616` 的 fresh-main 已满足；WI-214 formal active | 五个只读命令含 help/invalid sentinel + `plan-check normal` 代表性 real-hook A/B；共享 hook 一例 partial-write；`init/link` valid/missing/dirty/no-project/no-checkpoint/config-lock/other-exception 分发零差异；三阶段双 Agent/Codex/CI/fresh-main |
