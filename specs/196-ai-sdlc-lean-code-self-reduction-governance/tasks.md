...
| T58 隔离 workitem 只读 adapter side effect（active） | standalone | GAP-15 | L2 / CC-05 | WI-213 PR #158 / merge `450d4988` 与 lifecycle PR #159 / merge `d5ad7616` 的 fresh-main 已满足；WI-214 formal active | 五个只读命令含 help/invalid sentinel + normal real-hook bytes/output/clean-tree RED→GREEN；`init/link` valid/missing/dirty/no-project/no-checkpoint/config-lock/other-exception 时序零差异；formal/implementation/lifecycle 双 Agent/Codex/CI/fresh-main |
...
mainline 与 fresh-main。WI-213 验证发现并登记 GAP-15；当前 T58/WI-214 formal active，只有其
implementation 与 lifecycle reconciliation fresh-main 后才允许创建一个 T66 implementation WI。该 WI 未完成 T61A 双
