       ...
       └─ T56 GAP-13 portable/lossless resume reconstruction（WI-208 / PR #143，已关闭）
            └─ T57 GAP-14 YAML quoted-scalar comment-policy false positive（WI-209 / PR #146，已关闭）
                 └─ T63 exact text-dedupe family（WI-210 / PR #149，completed_reduction）
                      └─ T63 exact mapping-dedupe family（WI-211，formal review candidate）
```
...
- **当前状态**：WI-205、WI-206、WI-210 已各关闭一个 family。WI-210 / PR #149 / merge `904fe5de`
  将 28 个 exact text-dedupe defs / 27 modules / 196 LOC 收敛为 1 个共享 body 与 28 个局部 alias，
  730 calls 不变；产品 raw 净删 213、non-empty 净删 161，fresh-main targeted/full/治理/clean-state 全绿。
  GAP-05 仍 active。WI-211 当前只冻结 10-module exact mapping-dedupe candidate：120 duplicate LOC、
  23 calls，spike raw net -122/non-empty net -104；该值在 implementation/closure 完成前不计入 ledger。
- **范围**：一次只选一个经语义审查的重复族。
