...
- 唯一收据：
  ...
  7787 bytes；SHA-256
  `ca6a9387ae5ca1b18ae55878412c4b41331fe5ce14b4c635e1c25d075e5c1f4e`；schema
...
## 12. Batch 2026-07-16-011：final review Round 1 FAIL 与 PowerShell/continuity 修复

### 12.1 Round 1 同目标双 FAIL

- 送审 target：HEAD `737349808e13da9f19605faae8f0f0b4f212b148`、tree
  `726507d1aebbd3c1072d926d4e742788aa84385c`、binary diff SHA-256
  `3fcc280e0449d0e928ff87f037f055083d0a2558fb002827738bbee1ef6048b0`、receipt
  `3c0269fd5f34063f70600659ca2b4497dd78752e1f17af1f3ce68cf71e090310`；两位 Agent 起止复算无漂移。
- Pascal / 精简直接性：`FAIL`。Important：formal 冻结 PowerShell≥7.3 runbook，但首版 receipt 只记录
  zsh 等价协议；CI 明确不执行 paired-tree，不能替代本地 frozen runbook。另指出 continuity/summary
  仍把已提交的 final Truth 列为下一步。
- Confucius / 兼容证明：`FAIL`。唯一 Important 同样是 continuity/summary 过时，会诱导后续执行者再次
  改变已送审 target。其余 Path/hash/equality/异常、76/78/full、raw tree、rollback、Truth 均通过。
- 两侧均明确产品 helper、RC-06、changed-file scope 与回退证明未发现问题；本轮修复只触及证据、
  execution/summary/continuity，不改 runtime 产品或冻结 formal。

### 12.2 frozen PowerShell runbook 实跑

- 系统 Homebrew PowerShell `7.6.1_1` 在 .NET 10.0.7 下启动即以
  `System.Text.RegularExpressions, Version=10.0.0.0` assembly load error 退出；Homebrew `7.6.3`
  因 macOS 27 无 bottle 无法升级。
- 从 PowerShell 官方 GitHub release 下载隔离版 `powershell-7.5.8-osx-arm64.tar.gz` 到 `/tmp`；官方
  SHA-256 `AFAC2B812C8187A3AE7E487BC75CBCD6082F0C062A8964FF5E69D4F95A6486C1` 校验通过；
  `/tmp/powershell-7.5.8/pwsh` 输出版本 `7.5.8`，满足 frozen ≥7.3 gate，未覆盖系统安装。
- 用 frozen `$ErrorActionPreference='Stop'`、`$PSNativeCommandUseErrorActionPreference=$true`、同一
  `/tmp/ai-sdlc-wi205-pwsh-shared`、外置 SHA-1 ODB、fresh index、空 attributes file、
  `GIT_ATTR_NOSYSTEM=1`、`core.autocrlf=false` 顺序执行五个样本：
  baseline-1/2、final candidate `73734980`、rollback `091f5d54`、restored candidate `c1db361b`。
- 五个样本均 `76 passed`；tree 均为
  `6e1803c945b2bcb954bc6655ad2615b1458619a1`、463 entries（418 regular +45 symlinks）；
  `diff-tree --binary` 均为空，allowlist=`[]`，Git 2.54.0、core.filemode/symlinks=`true`。
- zsh 结果 `a9b62108...` 保留为早期同协议预检；因 absolute shared root 不同，OID 不要求跨 root
  相等。最终 authoritative local paired evidence 为上述 PowerShell `6e1803c9...` tuple。

### 12.3 receipt 与 continuity 修复

- receipt 已切换为 PowerShell authoritative evidence，并保留 implementation/final-sample/revert/
  restored revision、官方 archive hash、完整 attributes/config 与 Truth 审计结果；7787 bytes；
  SHA-256 `ca6a9387ae5ca1b18ae55878412c4b41331fe5ce14b4c635e1c25d075e5c1f4e`；JSON 校验 PASS。
- state/scoped handoff 与 development summary 删除“已完成 final Truth 仍列为下一步”的矛盾；本次
  source evidence 变更提交后重新执行 Program Truth sync/audit，再送 Round 2 同哈希双审。
