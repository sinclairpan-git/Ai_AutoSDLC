...
`hashlib.sha256(ast.dump(ast.Module(body=node.body, type_ignores=[]), include_attributes=False).encode()).hexdigest()`
计算的完整 body digest 均为
...
3. AST/non-empty 算法切片从 108 LOC 降为 8 LOC；固定 13 个产品文件的 raw diff additions≤23、
   deletions≥108、net≤-85；纳入 RC-06 代码预算的产品、顺序 test 与两条 root inventory expectation
   raw additions 合计≤27，等于 cap 27；formal/truth/continuity 文档不冒充产品或 test source。
...
- 创建五件套后，将版本敏感的 root source-inventory expectation 从 1076/1076 精确更新为
  1081/1081，并将 close layer 从 204/204 更新为 205/205；只替换两个既有 assertion line，不新增
  test function。
...
在 `_artifact_paths.py` 保留同一算法合同，并把反向 `if ...: continue` 等价收敛为正向
`if path not in seen:`，12 个 module 显式导入同名私有 helper。membership、`seen.add`、
`unique.append` 的执行条件/顺序、hash/equality 调用和异常传播不变。它有 13 个现有调用点，满足
LP-02；不需要 public API、配置或 adapter。固定 13 个产品文件的 raw diff 冻结为
additions≤23、deletions≥108、net≤-85。
...
| RC-03 | 固定 13 产品文件 raw additions≤23；新增产品文件=1；公共抽象=0 |
| RC-04 | AST 算法 108→8（-92.6%）、complexity 36→3、fan-out 36→3；产品 raw deletions≥108、net≤-85 |
| RC-05 | 不允许 shadow 双实现；单提交替换；产品 raw additions≤23 |
| RC-06 | 纳入代码预算的产品 raw additions≤23 + 既有 order test raw additions=2 + 两条 root inventory expectation replacements raw additions=2 + handwritten harness/normalizer source=0；product/test source 合计≤27；replacement deletions=2 只作披露、不抵扣；formal/truth/continuity 文档与唯一 generated receipt 不计 handwritten product/test source LOC，receipt <2 MiB |
| RC-07 | 新文件 <400；唯一函数 <50；13 个现有调用点；无 public export |
| RC-09 | 需要选项/路径规范化/类型分支、新 test function/file/custom harness，order test additions≠2、inventory expectation additions/deletions≠2/2、product/test source additions>27、产品 additions>23、Git tree baseline 不稳定或需非空 allowlist 时停止 |
...
- Path/order/error 由既有 test 内两行顺序断言、baseline/candidate 完整 AST body digest、14 个既有 dedupe tests 与
...
- **FR-02**：helper 必须按首次出现顺序返回唯一 `Path`，且只使用当前 `Path` hash/equality；candidate
  必须使用等价的 `if path not in seen:` 正向分支，不增加 hash/equality 调用或错误包装。
...
- **FR-07**：代码改动不得超出冻结的 helper + 12 imports/local-body deletion + 既有 test function
  两行断言 + root inventory/close 两条 expectation replacement；truth/continuity 与唯一 generated
  scoped receipt 除外。
...
- helper 出现选项、额外分支、协议、类或 public export，产品 raw additions>23，product/test source raw
  additions>27，或顺序保护不是既有 function 内精确两行、inventory/close expectation 不是两行替换、
  出现新 test function/file/harness，即按
  RC-09 No-Go。

## 8. 成功标准

- **SC-01**：12 份实现收敛为 1 份，13 个调用点全部复用 canonical helper。
- **SC-02**：AST 算法 108→8、complexity 36→3、fan-out 36→3；固定 13 产品文件 raw
  additions≤23、deletions≥108、net≤-85；纳入 RC-06 的 product/test source additions≤27。
