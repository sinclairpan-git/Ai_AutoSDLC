...
`_dedupe_paths` body。canonical helper 用等价正向 membership 分支再减一行。实施只包含一个新模块、
12 条 import、12 个本地 body 删除、现有 dedupe test function 内两行 direct-binding 顺序断言，
以及 root inventory/close 两条 literal replacement；不新增 test function/file 或 harness，不改变
artifact/CLI/schema/runtime behavior。
...
- **约束**：RC-04/06/07/09/10；产品 raw additions≤23；既有 order test raw additions=2；root
  inventory/close replacements raw additions=2/deletions=2；纳入 RC-06 的 product/test source
  additions≤27=cap；formal/truth/continuity 文档不计入产品或 test source
...
| 可量化净减重 | AST 108→8、complexity 36→3；产品 raw additions≤23、deletions≥108、net≤-85 |
...
- `tests/integration/test_repo_program_manifest.py`（inventory 1076/1076→1081/1081；close 204/204→205/205）
...
1. 新增 `_artifact_paths.py`，保留 typed `Path` 合同并使用等价的 `if path not in seen:` 正向分支，
   candidate body=8 LOC；不加选项、包装器、类或多余注释。
...
3. Ruff 排序 imports；Phase 1 的两行断言继续通过原模块 private binding 观察 imported canonical
   helper；Phase 2 不再修改测试，最终 test diff 仍精确为 order `+2/0` 与 root truth `+2/-2`；
   artifact payload 不变。
4. Appendix A AST gate 复算 defs 12→1、算法 108→8、complexity 36→3、fan-out 36→3、
   calls=13；baseline digest=`fc689b7a...be9b4`，candidate digest=`aec166ee...8c91`；raw ledger 与
   source-addition gate 复算固定 changed-file set。

**门禁**：产品 raw additions>23、deletions<108、net>-85、product/test source additions>27、
实现数不为 1、调用点不为 13、body digest 不等于冻结的 baseline/candidate 值或 changed files 越界时
按 RC-09 停止。
...
| Path equality/hash | 正向分支可观察语义等价 + tests | 三平台 CI |
...
1/8/3/3/13/imports 12；baseline digest 为 spec §1 的完整值，candidate digest 必须为
...
候选形成 local commit 后执行；统计冻结的产品目录、order test 与 root inventory truth test，避免
未跟踪 helper/保护成本被漏算。输出必须满足 product additions≤23/deletions≥108/net≤-85、
order test additions=2/deletions=0、inventory/close replacements additions=2/deletions=2、纳入 RC-06 的
product/test source additions≤27；formal/truth/continuity 文档不计入该代码预算：

```text
git diff --numstat e4f395e3 HEAD -- src/ai_sdlc/generators tests/unit/test_frontend_contract_artifacts.py tests/integration/test_repo_program_manifest.py
```
...
无。若实现需要新 test function/file/harness、order test 增量不等于 `+2/0`、root truth test
replacement 不等于 `+2/-2`、helper 选项或 path normalization，按冻结的 RC-09 直接停止并回到
formal，不在 execute 中扩大范围。
