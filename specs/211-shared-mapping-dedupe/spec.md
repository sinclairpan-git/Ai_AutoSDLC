...
- 10 个模块局部 callable 从 10 个对象收敛为同一个 imported private object，因而 `id`、`__module__`、
  `__code__`、源码位置和 traceback frame 统一到 `ai_sdlc.utils.helpers`；
- 7 个仅供本地 helper 使用的模块级 `json` binding 被删除；另外 3 个模块因其他 JSON 行为继续保留；
- `utils.helpers` 新增内部标准库 `json` binding。

这些差异只允许作用于未被外部消费的 private introspection surface。已验证外部 private import、attribute
consumer、wildcard consumer 与 monkeypatch consumer 均为 0。不得借此批准结果、异常 type/message、事件顺序、
artifact 字节、schema、CLI transcript 或状态真值差异。
...
1. **Given** 10 个基线 binding 与 14 类成功/失败输入，**When** 对 candidate、revert、reapply 运行差分，
   **Then** 每个运行时的 140 个 observation 与对应 baseline 完全一致。
2. **Given** 非 dict、key 顺序不同、Unicode、嵌套对象、不可序列化值、混合 key 与循环引用，**When**
   调用任一 binding，**Then** 结果、异常 type/message、事件顺序与浅复制语义不变。
...
1. **Given** reviewed candidate commit/tree，**When** 在 disposable clone revert implementation commit，
   **Then** tree OID 精确等于 baseline，结构、140 observations、direct tests 与 cold imports 全部通过。
2. **Given** revert 状态，**When** reapply reviewed implementation commit，**Then** tree OID 精确恢复 candidate，
   同一证据再次通过；随后 full、跨平台 CI、Codex 与 fresh-main acceptance 全绿。

## 4. Differential 与 TDD 合同

### 4.1 T61A baseline

- 10 bindings × 14 cases = 140 observations；case 名固定为 `none`、`empty`、`duplicate`、`key_order`、
  `invalid_values`、`unicode`、`nested_duplicate`、`int_keys`、`falsy_distinct`、`unserializable`、
  `cyclic`、`mixed_key_sort`、`truthiness_events`、`dict_subclass_shallow`；
- 当前 Python 3.11 spike baseline/candidate digest 均为
  `05b7908685c415a6dada0b1530ca0bd310afb4f8ca4b950343752a5ea6643aab`；跨 Python/OS 只要求同环境
  old/new 完全一致，不把不同解释器的异常文案摘要强制为同一值；
...
只在现有 `tests/unit/test_frontend_contract_observation_provider.py` 增加一个 binding identity test。
...
### 4.3 T61B candidate / rollback / reapply

唯一结构化回执：
`.ai-sdlc/work-items/211-shared-mapping-dedupe/t61-differential-rollback-receipt.json`。它必须绑定 baseline、
candidate、revert、reapply commit/tree，记录 140-observation digest、结构/调用 digest、72 cold imports、
product/test LOC 与受保护文件 hash；baseline/revert 测试数为 direct 103 / impact 1162，增加唯一 identity
test 后 candidate/reapply 为 direct 104 / impact 1163。receipt 不构成第二套 schema 或产品能力。
...
| GAP-11 source inventory | 实现新增 source=0；formal/closure 只增加 canonical WI source | unexpected missing/unmapped、source 集合或 truth state 漂移即停止并重开 |
...
| RC-06 | 产品 non-empty additions≤23；TDD test additions≤20；truth test机械 additions≤2；合计≤45，hard cap=49；0 fixture/新测试文件 |
...
| RC-10 | before/after、TDD、140 diff、103→104 direct、1162→1163 impact、72 imports、rollback/reapply、full/platform/fresh-main |
...
- **FR-211-006**：baseline/revert 必须通过 140 observations、103 direct、1162 impact 与72 cold imports；
  candidate/reapply 必须通过同一行为集加唯一 identity test，即104 direct、1163 impact，并通过full suite。
- **FR-211-007**：rollback/reapply tree OID 与唯一 receipt 必须绑定 reviewed candidate identity。
...
- **SC-211-003**：TDD RED 根因精确；candidate 为140 observations、104 direct、1163 impact、72 cold imports、full、
  Ruff、constraints、validate、truth 全绿。
- **SC-211-004**：rollback tree=baseline，reapply tree=candidate；receipt hash 与 reviewed identity 一致。
