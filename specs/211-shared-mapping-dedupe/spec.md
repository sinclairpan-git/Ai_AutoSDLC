...
三个 AST digest 均用当前 Python 的 `ast.parse` 和 UTF-8 source 复算：full payload 为目标顶层
...
payload 均以 UTF-8 做 SHA-256；不得把行号、文件位置或格式化空白放入 digest。
...
- `plan.md` §3.3 的 `wi211-t61-corpus-v1` 是唯一 canonical factory、事件、规范化、排序与 SHA-256
  recipe；当前 Python 3.11 formal baseline/candidate 均为140 observations、digest=
  `2657ee073f131d0760ee1b751d32f5f71c2f9afe30a7f05bc261487ea8c1d695`，不再把不可重建的 preliminary
  spike digest 当作验收值；跨 Python/OS 只要求同环境 old/new JSONL 字节完全一致，不把不同解释器的异常
  文案摘要强制为同一值；
