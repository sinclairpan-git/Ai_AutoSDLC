...
RED 必须只因两个代表模块的 local function 对象不相同；不得以 import、collection 或 fixture 错误代替。
全部10个 binding 的 identity 由独立结构门禁验证。GREEN 只允许：
...
104 / 1163。后续 evidence commit chain 只允许 receipt、AGENTS.md 强制的 root/scoped handoff 与必要机械
