...
- 产品只改 `src/ai_sdlc/core/verify_constraints.py`；新行为断言只改 `tests/unit/test_verify_constraints.py`；
  两个既有 integration 文件仅可补显式 framework identity fixture setup。
- 产品 raw additions ≤80；新增私有 helper ≤2；不新增模块、配置、依赖或公开抽象。
...
| `src/ai_sdlc/core/verify_constraints.py` | 私有 scope 判定及三入口机械分流 |
| `tests/unit/test_verify_constraints.py` | 身份矩阵、Agent Store、编号碰撞、门禁保留回归 |
...
6. 现有 PrimeVue consumer report/context 保留；
7. framework checkout 与既有 framework drift 测试保持。

在产品未改时，至少 Agent Store 与 consumer metadata/编号碰撞测试必须 RED；既有框架测试保持 GREEN。
所有新行为断言仍局限在 unit 文件。上述两个 integration 文件只能在逐个明确的 framework fixture 内创建
...
5. 不建立 registry/dispatcher/role class；若 raw additions 超过 80，停止并压缩，不放宽合同。
6. 不增加“缺少双信号仍按 framework”之类的产品兼容 fallback；fixture 必须显式表达其仓库身份。
...
```powershell
uv run pytest -q tests/unit/test_verify_constraints.py
