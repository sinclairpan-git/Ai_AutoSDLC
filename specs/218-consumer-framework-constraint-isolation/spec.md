...
Framework checkout 中既有 WI `014` canonical attachment/warning 及其 `SDLCRunner`、`run` CLI summary
呈现必须保留；consumer 即使使用相同编号，也不得经 canonical context、runner 重复注入或 CLI summary
重建这些 framework-owned 结果。
...
sources。不得仅过滤 blocker 而保留虚假的框架检查元数据；下游 `SDLCRunner` 不得重复注入 canonical
warning，`run` CLI summary 必须复用同一 `_repository_scope` 判定，不能根据内部 WI 编号重新推断框架身份。
`ProgramService` 的通用状态与执行路径不在本项修改范围。

## 4. 代码纯洁性合同

| 合同 | 硬约束 |
|---|---|
| LC-01 | implementation 产品代码精确只允许修改 `src/ai_sdlc/core/verify_constraints.py`（scope/canonical context，并回收行数）、`src/ai_sdlc/core/runner.py`（只删除重复注入）、`src/ai_sdlc/cli/run_cmd.py`（只复用同一 `_repository_scope` 做 summary guard） |
| LC-02 | implementation 行为测试精确只允许扩展 `tests/unit/test_verify_constraints.py`、`tests/unit/test_runner_confirm.py`、`tests/unit/test_run_cmd.py`、`tests/integration/test_cli_run.py`；另保留两个既有 integration 文件仅补显式 framework identity fixture setup 的例外 |
...
| LC-05 | LC-01 三个产品文件合计 raw additions 硬上限 80 行；新增 helper 仍以 1 个为目标、最多 2 个；超限必须停止并重新压缩设计 |
...
新增 `014` 验收必须同时包含 consumer canonical context/runner/CLI summary 的 negative assertions，以及
带双身份信号的 framework positive fixtures；不得把该覆盖扩展为新的 harness 或 ProgramService 行为测试。
...
- **FR-218-006**：consumer 的任何内部 WI 编号碰撞不得激活 Ai_AutoSDLC 历史附件、warning 或 release gate，
  也不得经 canonical context、`SDLCRunner` 或 `run` CLI summary 重建 framework attachment/warning；framework
  checkout 的既有 `014` 行为必须保留。
...
- **SC-218-003**：routing-census 证明 consumer 三入口调用全部 common、零 framework helper；framework 调用
  ...
  fixture 非空覆盖 attachment/check object/source/context；consumer 均无框架污染。`014` consumer fixture
  必须证明 canonical context、`SDLCRunner` 和 `run` CLI summary 均不重建 attachment/warning，双身份信号的
  framework positive fixtures 必须证明既有 `014` 行为不变。
...
- **SC-218-006**：生产代码满足 LC-01～LC-07；三个允许产品文件合计 raw additions ≤80，新增 helper ≤2。
