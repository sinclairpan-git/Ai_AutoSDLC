...
- **文档子产物（已落地，无代码）**：
  1. [`USER_GUIDE.zh-CN.md`](../../USER_GUIDE.zh-CN.md) **§2.1**（DoD / 计划状态 / 指向 plan-check 规格与 PR 清单）。
...
- **验收标准（AC）**：
  1. 默认仅扫描 `specs/<WI>/` 下 `*.md` 与 **白名单**（至少含 `docs/pull-request-checklist.zh.md`、`USER_GUIDE.zh-CN.md`；路径以 `--wi` 解析结果为锚）。
...
> **Task 6.42 完成（2026-03-28）**：[`GitClient`](../../src/ai_sdlc/branch/git_client.py) 已新增仓库级 Git 写锁、写命令分类与 `.git/index.lock` active/stale 判定；`add_and_commit()` 固化为 `git add -> git status/diff -> git commit` 顺序，`push()` 明确为后置单独步骤；[`multi-agent.md`](../../src/ai_sdlc/rules/multi-agent.md)、[`batch-protocol.md`](../../src/ai_sdlc/rules/batch-protocol.md) 与 [`docs/框架自迭代开发与发布约定.md`](../../docs/框架自迭代开发与发布约定.md) 已同步写明 Git 写命令不得并行。定向回归 **58 passed**，`FD-2026-03-28-004` 已关闭。
...
> **Task 6.43 完成（2026-03-28）**：已完成两轮产品化。第一轮将“未 commit 收口”固化为 `close-check.git_closure` blocker；第二轮将 `docs-only / rules-only / code-change` 最小 fresh verification 画像正式写入 `verification.md`、PR checklist 与 execution-log 模板，并让 `close-check` 校验 latest batch 的画像/命令匹配、`verify constraints` 校验 verification profile surface 完整性。定向回归 **55 passed**，仓库级 `close-check --all-docs` 与 `verify constraints` 已通过，`FD-2026-03-24-003` 已关闭。
...
> **Task 6.44 完成（2026-03-28）**：`task_ac_checks` 已补 doc-first / requirements-first 共享判定，`ExecuteGate` 现在会对目标任务为“仅文档 / 仅需求沉淀 / 先 spec-plan-tasks”时报 `doc_first_prerequisite`，并显式指出禁止默认改动 `src/` 非 Markdown 代码与 `tests/` 路径；`verify constraints` 同步校验 `pipeline.md` / `agent-skip-registry.zh.md` 的术语 surface 与 `tasks.md` 中 doc-first 任务范围。定向回归 **101 passed**，`FD-2026-03-26-001` 已关闭。
...
> **Task 6.45 完成（2026-03-28）**：第二波 backlog 的 `FD-2026-03-28-004`、`FD-2026-03-24-003`、`FD-2026-03-26-001` 已在 backlog / 本文件 / execution-log 对齐为同一真值并全部关闭；仓库级 `verify constraints` 与 `workitem close-check --wi specs/001-ai-sdlc-framework --all-docs` 已通过，Batch 16 收口完成。

### Task 6.46 — Close Gate 可信收尾补证（FD-2026-04-14-001）

- **依赖**：Task 6.45。
- **输入**：[`src/ai_sdlc/core/runner.py`](../../src/ai_sdlc/core/runner.py)、[`src/ai_sdlc/cli/sub_apps.py`](../../src/ai_sdlc/cli/sub_apps.py)、[`src/ai_sdlc/core/close_check.py`](../../src/ai_sdlc/core/close_check.py)、[`tests/unit/test_runner_confirm.py`](../../tests/unit/test_runner_confirm.py)、[`docs/framework-defect-backlog.zh-CN.md`](../../docs/framework-defect-backlog.zh-CN.md)。
- **验收标准（AC）**：
  1. `execute_progress` 为空/不完整但 `workitem close-check` PASS 时，Close Gate 可依据其 `tasks_completion` / `verification_profile` 可信补证，不再阻断。
  2. `close-check` FAIL 时仍保持阻断，且不会伪造或篡改 `execute_progress`。
  3. DoneGate 上下文能够明确标注 close-check 补证来源，用于收口审计。
- **验证**：Close Gate fallback 单测 + `workitem close-check` + `verify constraints`。

> **Task 6.46 完成（2026-04-14）**：`runner` 与 `cli` close/done 路径已在缺失 `execute_progress` 时回落到 `close-check` 结果映射，补齐 `all_tasks_complete/tests_passed` 与审计标记；新增 `test_close_context_attests_with_close_check_when_execute_progress_missing` 覆盖该场景。全量 **`uv run pytest -q`**、`uv run ruff check .` 与 `uv run ai-sdlc verify constraints` 已通过。

### Task 6.47 — Legacy adapter path 迁移兼容（FD-2026-04-14-002）

- **依赖**：Task 6.46。
- **输入**：[`src/ai_sdlc/integrations/ide_adapter.py`](../../src/ai_sdlc/integrations/ide_adapter.py)、[`tests/unit/test_ide_adapter.py`](../../tests/unit/test_ide_adapter.py)、[`USER_GUIDE.zh-CN.md`](../../USER_GUIDE.zh-CN.md)、[`docs/framework-defect-backlog.zh-CN.md`](../../docs/framework-defect-backlog.zh-CN.md)。
- **验收标准（AC）**：
  1. 项目只有旧路径（legacy）时，`apply_adapter` 需在不覆盖用户现有 canonical 文件的前提下补齐 canonical path。
  2. `status/ingress` 能提示已检测到 legacy 文件，但不会把 legacy 误判为已 materialized 的 canonical。
  3. 用户手册明确升级后 canonical path 与迁移行为，避免误解。
- **验证**：legacy 迁移单测 + 手册更新 + `verify constraints`。

> **Task 6.47 完成（2026-04-14）**：`ide_adapter` 增加 legacy path 检测与迁移回填，`ingress` 在仅 legacy 存在时给出明确 degrade 说明；新增 `test_migrates_legacy_vscode_adapter` 回归；用户指南补充升级兼容提示。全量 **`uv run pytest -q`**、`uv run ruff check .` 与 `uv run ai-sdlc verify constraints` 已通过。

### Task 6.48 — Close-check 允许 pipeline state dirty 白名单（FD-2026-04-14-003）

- **依赖**：Task 6.47。
- **输入**：[`src/ai_sdlc/core/close_check.py`](../../src/ai_sdlc/core/close_check.py)、[`tests/unit/test_close_check.py`](../../tests/unit/test_close_check.py)、[`docs/framework-defect-backlog.zh-CN.md`](../../docs/framework-defect-backlog.zh-CN.md)。
- **验收标准（AC）**：
  1. 仅 `.ai-sdlc/state/checkpoint.yml(.bak)` dirty 时，`workitem close-check` 不应因 git_closure 失败。
  2. 若存在其他未提交变更，close-check 仍需 BLOCK。
  3. 回归测试覆盖上述白名单场景。
- **验证**：`tests/unit/test_close_check.py` + `verify constraints` + `ai-sdlc run`。

> **Task 6.48 完成（2026-04-14）**：`close_check` git_closure 过滤 pipeline state 白名单，并新增 `test_close_check_ignores_checkpoint_state_dirty_files` 回归；`uv run pytest -q`、`uv run ruff check .` 与 `uv run ai-sdlc verify constraints` 通过，`uv run ai-sdlc run` 已可完成 close 阶段。
...
| 17 | P1 兼容收口 | 2 | 6.46→6.47 |
| 18 | P1 close-check 稳定性 | 1 | 6.48 |
| **合计** | **0-18 + 增量 + P1×9 + P0 gap×3** | **88** | |
...
| M16：001 升级兼容与可信收尾收口 | Task **6.46～6.47** 完成 | Close Gate 可信补证、legacy adapter 迁移与用户手册升级提示 |
| M17：001 close-check 稳定性收口 | Task **6.48** 完成 | git_closure 允许 pipeline state 白名单，run close 可完成 |
