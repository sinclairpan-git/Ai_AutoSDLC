...
本计划处理的是“在 `015` 已冻结的 MVP recipe baseline 和 `067` 已落地的 P1 expanded kernel truth 之上，如何正式进入 P1 page recipe expansion”的第二条 child baseline。当前 formal baseline 已完成；在不扩大到 diagnostics / provider/runtime / root sync 的前提下，当前允许唯一的首批 implementation slice，把同一份 recipe truth 落到 Kernel model 与定向 unit tests。当前已存在的前置真值包括：
...
在这些 truths 已经落稳的前提下，`068` 的当前目标有五件事：
...
- 为后续 Kernel 模型 / 生成工件 / 测试切片提供单一 canonical baseline
- 在 formal baseline 完成后，仅以首批 implementation slice 将 recipe truth 落到 model 与定向 unit tests

`068` 的 formal baseline 已完成；当前批次只允许 **Batch 5 page recipe model expansion slice**。当前写面只放行 `src/ai_sdlc/models/frontend_ui_kernel.py`、`src/ai_sdlc/models/__init__.py`、`tests/unit/test_frontend_ui_kernel_models.py` 与 `tests/unit/test_frontend_ui_kernel_artifacts.py`。`068` 不实现 provider/runtime，不改 `src/ai_sdlc/generators/frontend_ui_kernel_artifacts.py`，不改 root `program-manifest.yaml` 或 `frontend-program-branch-rollout-plan.md`，也不生成 `development-summary.md`。
...
- 当前 implementation slice 只允许写入 2 个 model 文件与 2 个 unit test 文件，不得提前 root sync
...
| MUST-5 产品代码与开发框架隔离 | `068` 仅在当前唯一 implementation slice 中修改 2 个 Kernel model 文件与 2 个 unit test 文件，不扩张到 generator、provider/runtime 或其他 `src/` / `tests/` |
...
### 当前唯一允许的 implementation slice 写面
...
上面的文件面构成当前 Batch 5 唯一允许的写面；`src/ai_sdlc/generators/frontend_ui_kernel_artifacts.py`、其他 `src/` / `tests/`、root truth 文件与下游 child 文档都不在当前授权范围内。
...
- `068` formal baseline 已完成；当前只允许唯一的 Batch 5 page recipe model expansion slice，不得顺手写 root `program-manifest.yaml` 或 rollout plan
...
| --- | --- | --- |
| `068` child baseline | 冻结 P1 recipe 标准本体、区域约束与状态期望，并在当前唯一 implementation slice 中把这些 truth 落到 Kernel model 与定向 unit tests | 不得重写 `067` 的 `Ui*` 语义，也不得扩到 generator、provider/runtime 或其他 `src/` / `tests/` |
...
**目标**：通过 docs-only 门禁验证 `068` 的 formal baseline，并完成 execution log 初始化。
**产物**：`task-execution-log.md`。
**验证方式**：`uv run ai-sdlc verify constraints`、`git diff --check`。
**回退方式**：仅更新文档归档，不引入新实现。

### Phase 5：Page recipe model expansion slice

**目标**：将 `spec.md` 已冻结的 P1 page recipe expansion truth 落到 frontend UI Kernel model，并保持 artifact materialization 继续消费该 truth。
...
### 工作流 D：Baseline validation and archive

**范围**：`verify constraints`、`git diff --check`、execution log append-only 归档。  
**影响范围**：`068` formal baseline 与当前 implementation slice 的可审计性。
**验证方式**：命令结果 + execution log。
**回退方式**：仅回退 docs 改动。

### 工作流 E：Page recipe model expansion slice

**范围**：新增 page recipe builder、`consumed_protocols` / `minimum_state_expectations` 约束与 artifact payload 承接。
**影响范围**：后续 `069` diagnostics / drift、future provider/runtime child 的 shared recipe truth。
...
**回退方式**：不新增 diagnostics / provider/runtime / root sync 行为。
...
| 当前状态诚实性 | `task-execution-log.md` review | `git status --short` |
| baseline 可读性 | `uv run ai-sdlc verify constraints` | `git diff --check` |
| page recipe model expansion correctness | `uv run pytest tests/unit/test_frontend_ui_kernel_models.py tests/unit/test_frontend_ui_kernel_artifacts.py -q` | recipe payload review |
