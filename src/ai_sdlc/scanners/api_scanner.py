...
PYTHON_ROUTE_PATTERNS = [
    ...
    re.compile(
        r"@\w+\.(?P<method>" + "|".join(HTTP_METHODS) + r")\s*\(\s*['\"](?P<path>[^'\"]+)['\"]"
    ),
...
def _scan_python_routes(path: Path, rel_path: str) -> list[ApiEndpoint]:
    ...
    for i, line in enumerate(lines, 1):
        for pattern in PYTHON_ROUTE_PATTERNS:
            ...
            if m:
                method_raw = m.group("method") if "method" in pattern.groupindex else "GET"
                if method_raw and method_raw not in HTTP_METHODS:
                    methods = [m2.strip().strip("'\"").upper() for m2 in method_raw.split(",")]
                ...
                endpoints.append(ApiEndpoint(
                    ...
                    line_number=i,
                    framework="fastapi" if "@" in line and "route" not in line.lower() else "flask",
                ))
    return endpoints
