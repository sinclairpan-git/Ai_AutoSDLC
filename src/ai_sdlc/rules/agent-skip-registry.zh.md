...
| **杜绝（流程）** | ① 用户指令含「**先文档 / 先需求 / 先 spec-plan-tasks**」，或当前任务属于「**仅文档 / 仅需求沉淀**」时，**禁止**默认修改 `src/`、`tests/`（除非任务本身为代码任务且 `tasks.md` 已勾选就绪）；② 先追加或更新 `spec.md` / `plan.md` / `tasks.md`，再进入实现批次；③ 将「规则过重/作用域过大」类反馈**先记入 spec（FR）与 tasks**，再开实现 PR。 |
