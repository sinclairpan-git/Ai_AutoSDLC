...
|------|----------|------|--------------------|
| `docs` | `feature/<WI>-docs`（legacy: `design/<WI>-docs`） | Stage 1-4 设计产物 | 否 |
| `feature` | `feature/<WI>-dev`、`feature/<WI>-*` | Stage 5-6 开发实现 | 否 |
...
main (或 develop)
  │
  ├─ feature/<WI>-docs          ← Stage 1-4 的设计产物（spec、plan、tasks 等）
  │    │
  │    └─ feature/<WI>-dev       ← Stage 5 EXECUTE 的代码实现（从 docs 分支归档后创建）
  │         │
  │         └─ 开发完成后合并回 main
  │
  └─ (下一个 Feature 重复上述结构)
...
| 分支 | 创建时机 | 内容 | 合并时机 |
|------|---------|------|---------|
| `feature/<WI>-docs` | Stage 1 REFINE 开始前 | spec.md, plan.md, research.md, data-model.md, contracts/, tasks.md, quickstart.md | Stage 4 VERIFY 通过后合并到 main |
| `feature/<WI>-dev` | Stage 5 EXECUTE 开始前 | 全部代码、测试、execution-log、development-summary | Stage 6 CLOSE 完成后合并到 main |

说明：

- `design/<WI>-docs` 仅作为历史仓库兼容命名保留；新分支默认使用 `feature/<WI>-docs`。
- `feature/<WI>-dev` 是推荐命名；如果项目已有稳定实现分支约定，必须在 checkpoint 与 close-out 证据中保持同名一致。
...
### Stage 1 REFINE 开始前：创建 docs 分支

```
1. 确认 main 分支上无未 commit 的变更
2. git checkout -b feature/<WI>-docs
3. 开始 Stage 1 工作
```

### Stage 1-4（在 docs 分支上执行）

```
每个阶段完成后在 docs 分支上 commit：
...
### Stage 4 → Stage 5 过渡：合并 docs 分支、创建开发分支

```
Stage 4 VERIFY 通过后：
  1. 确认 docs 分支上所有变更已 commit
  2. 合并 docs 分支到 main：
     git checkout main
     git merge feature/<WI>-docs --no-ff -m "docs(WI): merge design artifacts"
  3. 从 main 创建开发分支：
     git checkout -b feature/<WI>-dev
  4. 开始 Stage 5 EXECUTE

为什么从 main 而不是直接从 docs 分支创建 dev 分支？
  → 保证 dev 分支的基线包含最新的 main 代码
  → 如果有其他 feature 已合并，dev 分支也能包含
```

### Stage 5-6（在 dev / feature 分支上执行）

```
每个批次在 `feature/<WI>-dev`（或项目约定实现分支）上 commit
Stage 6 完成后，dev / feature 分支可合并到 main
```

### Stage 6 完成后：合并开发分支

```
1. 确认 dev / feature 分支上所有变更已 commit
2. 合并到 main：
   git checkout main
   git merge feature/<WI>-dev --no-ff -m "feat(WI): complete implementation"
3. 对当前 work item 相关分支写 disposition：
   □ merged
   □ archived（需说明为何保留且为何不等于主线真值）
   □ deleted（需说明删除前是否已 merged）
4. 如已 merged 且无需保留，删除分支/移除 worktree：
   git branch -d feature/<WI>-docs
   git branch -d feature/<WI>-dev
```
...
场景 B：从 docs 分支切换到 dev / feature 分支（Stage 4 → 5 过渡）
  1. 确认 docs 分支已合并到 main（git log --oneline main | grep WI）
  2. 确认设计产物在 dev / feature 分支上可访问：
     □ specs/NNN/spec.md 存在
     □ specs/NNN/plan.md 存在
     □ specs/NNN/tasks.md 存在
     □ specs/NNN/data-model.md 存在（如适用）
  3. 确认 tasks.md 内容与 docs 分支上的一致（无丢失）
  4. 确认测试框架可运行（如已有测试配置）
...
当项目已初始化（宪章存在），处理新 Feature 时：
  1. 确认在 main 分支上
  2. git pull（如有远程仓库）
  3. 创建 feature/<WI>-docs（legacy 仓库可兼容 design/<WI>-docs）
  4. 后续流程同标准流程
```
...
当同时开发多个 Feature 时：
  main
   ├─ feature/001-xxx-docs → feature/001-xxx-dev
   └─ feature/002-yyy-docs → feature/002-yyy-dev

规则：
  - 每个 Feature 有独立的分支对
  - docs 分支合并到 main 时需检查冲突
  - dev / feature 分支合并到 main 时需做全量回归
...
feature:
  ...
  design_branch: "feature/001-employee-signup-core-docs"
  feature_branch: "feature/001-employee-signup-core-dev"
  current_branch: "feature/001-employee-signup-core-docs"  # 当前活跃分支
```

恢复时，根据 `current_branch` 自动切换到正确的分支。`design_branch` 字段名出于兼容原因保留，但其值应指向当前 docs 分支。
