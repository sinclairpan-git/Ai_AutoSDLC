...
22. **Requirement 专家评审必须有界**：新建 Requirement 进入 `needs_review` 后，先运行只读 `ai-sdlc loop requirement review`，再按返回的一个主角色和至多一个风险角色分别进行独立只读评审。宿主只生成临时 `RequirementReviewExecution`；修订或冻结时用 `--review-result-file` 当次校验，不得把 findings、PASS 或临时 execution 写入 Loop artifact。只有当前 clean execution 加用户 `--yes` 可以 freeze，实质修订最多进入第二轮；旧 intake 继续显式走 legacy 兼容路径。
