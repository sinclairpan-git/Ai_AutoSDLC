...
def check_governance(
    ...
    *,
    ...
    governance_path = root / AI_SDLC_DIR / "work-items" / work_item.work_item_id / "governance.yaml"
    checks.append(GateCheck(
    ...
    if expected_branch:
        try:
            ...
            checks.append(GateCheck(
                name="branch_check",
                passed=branch_ok,
                message="" if branch_ok else f"Expected {expected_branch}, got {current}",
            ))
    ...
    checks.append(GateCheck(
        ...
        passed=ai_ok,
        message="" if ai_ok else f"AI decisions ({ai_decisions_count}) >= threshold ({MAX_AI_DECISIONS})",
    ))
