    ...
    def check(self, context: dict[str, Any]) -> GateResult:
        ...
        if spec_exists:
            ...
            has_clarification = bool(
                re.search(r"\[NEEDS[_ ]CLARIFICATION\]", content)
            )
            checks.append(GateCheck(
                name="no_needs_clarification",
                passed=not has_clarification,
                message="" if not has_clarification else "Found NEEDS_CLARIFICATION markers",
            ))
        else:
            for name in ("user_stories_present", "functional_requirements", "no_needs_clarification"):
                checks.append(GateCheck(name=name, passed=False, message="spec.md missing"))
