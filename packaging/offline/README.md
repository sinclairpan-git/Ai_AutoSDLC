...
>
> 发布入口口径：
> - Windows 使用 `ai-sdlc-offline-0.7.1.zip`
> - Linux/macOS 使用 `ai-sdlc-offline-0.7.1.tar.gz`
...
- 发布 `v0.7.1` 后，可运行 GitHub Actions 工作流 [release-artifact-smoke.yml](/Users/sinclairpan/project/Ai_AutoSDLC/.github/workflows/release-artifact-smoke.yml) 从 GitHub Release 下载正式 `.zip` / `.tar.gz` 资产并执行安装冒烟；该证据证明的是发布资产，而不是当前源码树临时构建产物
