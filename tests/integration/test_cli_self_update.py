...
def _env(tmp_path, *, channel: str = "github-archive") -> dict[str, str]:
    return {
        ...
        "AI_SDLC_UPDATE_ADVISOR_TEST_LATEST_VERSION": "v0.7.1",
...
def test_self_update_evaluate_json_reports_actionable_github_archive(
    ...
    assert payload["upstream_latest_version"] == "0.7.1"
    assert payload["channel_latest_version"] == "0.7.1"
...
def test_interactive_cli_renders_update_notice_once(tmp_path) -> None:
    ...
    assert "ai-sdlc self-update instructions --version 0.7.1" in first.output
