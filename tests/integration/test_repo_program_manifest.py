...
def test_root_program_manifest_covers_specs_and_host_ingress_canonical_evidence() -> None:
    ...
    release_paths = {
        ...
        *(f"docs/releases/v0.9.{patch}.md" for patch in range(0, 10)),
    }
    release_registry = {
    ...
    assert (inventory.state, inventory.total_sources, inventory.mapped_sources, inventory.unmapped_sources, inventory.missing_sources) == ("complete", 1190, 1190, 0, 8)
    assert (inventory.layer_totals["close"], inventory.layer_materialized["close"]) == (226, 218)
    assert inventory.layer_totals["release"] == 45
