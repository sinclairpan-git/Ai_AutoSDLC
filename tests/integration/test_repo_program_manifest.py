...
def test_root_program_manifest_covers_specs_and_host_ingress_canonical_evidence() -> None:
    ...
    frontend_capability = next(item for item in manifest.capabilities if item.id == "frontend-mainline-delivery")
    adapter_capability = next(item for item in manifest.capabilities if item.id == "agent-adapter-verified-host-ingress")
    release_paths = {
    ...
    capability_audit_states = {
        item.capability_id: (item.audit_state, tuple(item.blocking_refs))
        for item in snapshot.computed_capabilities
    }
    ...
    assert capability_audit_states == {
        "frontend-mainline-delivery": ("ready", ()),
        "agent-adapter-verified-host-ingress": ("ready", ()),
    }
    assert missing_entry_warnings == []
    assert tuple(frontend_capability.required_evidence.truth_check_refs) == (
    ...
    assert (tuple(adapter_capability.required_evidence.truth_check_refs), tuple(adapter_capability.required_evidence.close_check_refs)) == (("specs/159-agent-adapter-canonical-consumption-proof-runtime-baseline", "specs/200-adapter-canonical-consumption-truth"), ("specs/121-agent-adapter-verified-host-ingress-truth-baseline", "specs/122-agent-adapter-verified-host-ingress-runtime-baseline", "specs/159-agent-adapter-canonical-consumption-proof-runtime-baseline"))
    assert all(any(ref.startswith(f"{work_item}-") for ref in adapter_capability.spec_refs) for work_item in range(160, 164))
