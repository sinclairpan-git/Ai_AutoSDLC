...
def test_root_program_manifest_covers_specs_and_host_ingress_canonical_evidence() -> None:
    ...
    assert (inventory.state, inventory.total_sources, inventory.mapped_sources, inventory.unmapped_sources, inventory.missing_sources) == ("complete", 1116, 1116, 0, 0)
    assert (inventory.layer_totals["close"], inventory.layer_materialized["close"]) == (212, 212)
