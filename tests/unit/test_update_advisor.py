...
def test_github_archive_installed_runtime_gets_actionable_notice(
    ...
    monkeypatch.setenv("AI_SDLC_UPDATE_ADVISOR_TEST_LATEST_VERSION", "v0.7.1")
    ...
    assert evaluation.upstream_latest_version == "0.7.1"
    assert evaluation.channel_latest_version == "0.7.1"
    assert NOTICE_LIGHT in evaluation.eligible_notice_classes
    assert NOTICE_ACTIONABLE in evaluation.eligible_notice_classes
    assert evaluation.upgrade_command == "ai-sdlc self-update instructions --version 0.7.1"
...
def test_ack_notice_records_notice_version(monkeypatch, tmp_path) -> None:
    ...
    ack = ack_notice(NOTICE_LIGHT, "0.7.1")
