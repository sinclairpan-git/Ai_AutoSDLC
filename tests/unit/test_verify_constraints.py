...
def test_release_docs_consistency_blocks_when_release_entry_docs_drift(
    ...
    (tmp_path / "README.md").write_text(
        ...
        "- Release notes: `docs/releases/v0.9.8.md`\n",
        encoding="utf-8",
    )
    release_notes = tmp_path / "docs" / "releases"
    release_notes.mkdir(parents=True, exist_ok=True)
    (release_notes / "v0.9.8.md").write_text("# AI-SDLC v0.9.8 Release Notes\n", encoding="utf-8")
    (tmp_path / "USER_GUIDE.zh-CN.md").write_text("v0.9.8\nWindows\nzip\nmacOS\nLinux\ntar.gz\n", encoding="utf-8")
    offline_dir = tmp_path / "packaging" / "offline"
    offline_dir.mkdir(parents=True, exist_ok=True)
    (offline_dir / "README.md").write_text(
        "v0.9.8\nai-sdlc-offline-0.9.8-windows-amd64.zip\n"
        "ai-sdlc-offline-0.9.8-macos-arm64.tar.gz\n"
        "ai-sdlc-offline-0.9.8-linux-amd64.tar.gz\n",
        encoding="utf-8",
    )
...
def test_release_docs_consistency_passes_when_release_entry_docs_align(
    ...
    (tmp_path / "README.md").write_text(
        ...
        "-AddToPath\n"
        "--add-to-path\n"
        "releases/download/v0.9.8\n",
        encoding="utf-8",
    )
    release_notes = tmp_path / "docs" / "releases"
    release_notes.mkdir(parents=True, exist_ok=True)
    (release_notes / "v0.9.8.md").write_text(
...
def test_release_docs_consistency_blocks_stale_windows_upgrade_smoke_version(
    ...
    mem.mkdir(parents=True)
    (mem / "constitution.md").write_text("# C\n", encoding="utf-8")
    (tmp_path / "README.md").write_text("v0.9.8\n", encoding="utf-8")
    workflows_dir = tmp_path / ".github" / "workflows"
    workflows_dir.mkdir(parents=True, exist_ok=True)
    (workflows_dir / "windows-offline-smoke.yml").write_text(
        "expected upgraded ai-sdlc version 0.9.8\n"
...
def test_beginner_guide_blocks_stale_release_guidance_wording(tmp_path: Path) -> None:
    ...
    (tmp_path / "USER_GUIDE.zh-CN.md").write_text(
        ...
        "No such command 'install'\n"
        "--upgrade-existing\n"
        "这份手册现在默认以**当前仓库源码版 / 当前发布版 `v0.9.8`** 为准。\n"
...
def test_beginner_guide_blocks_existing_project_init_without_cd_back(
    ...
    (tmp_path / "USER_GUIDE.zh-CN.md").write_text(
        ...
        "No such command 'install'\n"
        "--upgrade-existing\n"
        "..\\ai-sdlc-offline-0.9.8-windows-amd64\\.venv\\Scripts\\python.exe -m ai_sdlc init .\n"
        "../ai-sdlc-offline-0.9.8-macos-arm64/.venv/bin/python -m ai_sdlc init .\n"
        "../ai-sdlc-offline-0.9.8-linux-amd64/.venv/bin/python -m ai_sdlc init .\n",
        encoding="utf-8",
    )
...
def test_readme_blocks_stale_release_guidance_wording(tmp_path: Path) -> None:
    ...
    (tmp_path / "README.md").write_text(
        ...
        "It is not a beginner-path setup step.\n"
        "`v0.9.8` is the current staged framework release.\n"
...
def test_root_agents_bounds_local_pr_review_convergence() -> None:
    ...
    required = (
        "one primary numeric review-budget unit",
        "75%",
        "25%",
        "A finding cluster is only for causal grouping and deduplication",
        "Each accepted candidate delta plus re-review",
        "at most two normal repair rounds",
        "one no-delta infrastructure rerun",
        "materially raises current risk or violates the frozen risk boundary",
        "Do not replace an approved architecture solely because",
        "surrender all remaining normal rounds",
        "across branches, PRs, names, and time",
        "approve-one-bounded-action",
        "unique_delta",
        "effort_cap",
        "terminal_outcome",
        "exact HEAD, base, diff",
        "required-check target",
        "one continuity surface",
